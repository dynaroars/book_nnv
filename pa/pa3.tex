...
A README template is provided. Please answer the questions in the template and include it in your submission. \url{https://github.com/dynaroars/book_nnv/blob/main/pa/templates/README_PA3.md}


\subsection{Part 1: Branch and Bound Algorithm Framework}

BaB systematically explores the space by maintaining a queue of subproblems and iteratively refining them until verification is complete or a counterexample is found.
...
You can use simpler approch like random sampling, or more advanced approach like PGD, FGSM, etc.
...
The \texttt{bound} function computes abstract output bounds using your abstraction from PA2 (\autoref{sec:pa2}).
...
\begin{itemize}
    \item Make sure you use the same set of DNNs and properties for the evaluation of both abstraction methods and the symbolic execution for fair comparison.
    \item Use at least 30 diverse DNN and property pairs in your evaluation.
    \item All text in your plots must be legible.
    \item If the DNNs you use are too small or too few to show the trend, you will receive a low score.
    \item Answer the questions in the README template.
\end{itemize}
\subsubsection{Test Your Implementation}

This function is used to test whether your BaB implementation is correct. 
Make sure there is \emph{at least one perturbed dimension}, e.g., input\_lower should not be equal to input\_upper, or otherwise, the input space is just a single point and no need to use abstraction.
...
\end{enumerate}
Note that, property\_fn can be an arbitrary function, make up your own property function (e.g., $y_0 > 0$ or $y_0 > y_1$, etc.,), just make sure to use \emph{same property function} for different methods so that the comparison is fair.
...
\begin{enumerate}
    ...
    \item For each network, measure:
        \begin{itemize}
            \item Symbolic execution time/result (PA1 \autoref{sec:pa1})
            \item Pure abstraction time/result (PA2 \autoref{sec:pa2})
            \item BaB verification time/result  
            \item Use appropriate timeouts (e.g., 30s)
        \end{itemize}
...
\begin{enumerate}
\item The zip file must be named \code{pa3-[yourname/groupname].zip} (1 submission per group)
\item One single main source file \code{pa3.py}. Indicate in the \code{README} file on how I can run it.
    \begin{itemize}
    \item Your code should use the provided class/function signatures as indicated above. E.g., \code{BranchAndBound}, \code{deduce}, \code{decide}, \code{bound}, \code{prune}...
    \end{itemize}
...
\begin{itemize}
\item 12 points — Core BaB implementation
    \begin{itemize}
    \item 3 points for correct \texttt{verify} algorithm implementation
    \item 3 points for \texttt{branch} function with proper input splitting
    \item 3 points for \texttt{bound} function integration with PA2 abstractions
    \item 3 points for \texttt{prune} function with correct pruning logic
    \end{itemize}
