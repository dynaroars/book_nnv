...
\begin{itemize}
    \item Make sure you use the same set of DNNs for the evaluation of both abstraction methods and the symbolic execution for fair comparison.
    \item Same requirement as PA1: use at least 25 DNNs of varying sizes (number of layers, number of neurons per layer, etc.) in your evaluation.
    \item A good proxy of precision is the width of the output bounds (i.e., upper - lower), but you can be creative with other metrics as well. Make sure to document your metric in the README.
    \item Make sure all text in your plots are legible.
...
\begin{enumerate}
\item the zip file must be named \code{pa2-yourname.zip} (1 submission per group)
\item One single main source file \code{pa2.py}. Indicate in the \code{README} file on how I can run it.
    ...
    \end{itemize}
\item a comepleted \code{README.md} file. You \emph{must} use the provided template and answer all questions in there.
