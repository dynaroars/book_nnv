...
\begin{example}[Collision Avoidance System]\label{ex:collision-safety}
...
\begin{example}[Robustness + Safety: Drone Controller]\label{exercise:drone}
...
\begin{exercise}[Global Consistency: Celsius and Fahrenheit]
...
% \begin{exercise}[Monotonicity in Multiple Inputs: Flight Risk]
...
% \begin{exercise}[Temporal Safety: Medical Monitoring]
...
% \begin{exercise}[Robustness in Speech Recognition]
...
% \begin{exercise}[Safety in Medical Diagnosis]
...
% \begin{exercise}[Consistency in Translation]
...
% \begin{exercise}[Monotonicity in Fairness]
...
\begin{exercise}[Counterexample: Safety in Vehicles]
...
\begin{exercise}\label{exercise:z3-properties}
...
Note that in this network the hidden neurons ($v_1 \ldots v_4$) use ReLU activation, but the output neurons ($y_1,y_2$) just use affine transformation.
...
\]
where $\phi_{in}$ is the \emph{precondition}, i.e., a condition over the inputs of $N$, and $\phi_{out}$ is the \emph{postcondition}, i.e., a condition over the outputs of $N$.%\footnote{$\phi_{in}$ is also called the precondition, and $\phi_{out}$ is called the postcondition.}.
...
\paragraph{Example (Complete pattern).}
Consider the toy DNN in~\autoref{fig:dnn-b} with two ReLU neurons $x_3, x_4$.
The activation pattern
\[
p = \{b_3=\mathsf{true},\;b_4=\mathsf{false}\}
\]
is complete because it fixes both neurons: $x_3$ is active  and $x_4$ is inactive.
This defines a full (complete) assignment to ReLU activation status.

\paragraph{Example (Partial pattern).}
The pattern
\[
p' = \{b_1 = \mathsf{true}\},
\]
is partial because it only fixes $n_1$ to be active.
It represents two complete patterns:
\[
\{b_1=\mathsf{true}, b_2=\mathsf{true}\},
\qquad
\{b_1=\mathsf{true}, b_2=\mathsf{false}\}.
\]
Thus, a partial pattern corresponds to a union of multiple assignments to ReLU activation status.
% show example of what it means like neuron x is active,  it would mean that the input is in this region, and the output is this linear function of the input. ...show some formula

% \paragraph{Why partial patterns matter.}
% During search, a solver may detect that a partial pattern already leads to unsatisfiability (e.g., $n_1=\mathsf{true}, n_2=\mathsf{false}$ contradicts input constraints).
% In this case, \emph{all complete patterns extending it can be safely pruned}, avoiding exponential blowup.

% \begin{example}[Unsatisfiable Partial Pattern]
% Suppose $n_1=\mathsf{true}$ implies $x_3 \ge 0$.
% Further, if $n_2=\mathsf{false}$ implies $x_4 < 0$, but combining these constraints with the input domain $x_1 \in [-1,1], x_2 \in [-2,2]$ is impossible, then the partial pattern
% \[
% \{n_1=\mathsf{true},\; n_2=\mathsf{false}\}
% \]
% is unsatisfiable.
% Since it has no consistent inputs, both complete patterns containing this assignment are eliminated at once.
% \end{example}

\tvn{may be need to add some trees}
...
To solve the NNV problem (\autoref{sec:nnv-problem}) modern DNN verifiers typically adopt the Branch-and-Bound (BaB) approach to search for activation pattern (\autoref{sec:activation-pattern}).

At its core, a BaB search consists of two main components: (branch) splitting into the problem smaller subproblems
using \emph{neuron splitting}, which decides boolean values representing neuron activation status,
...
%First, the T-solver creates a linear constraint system from the input assignment $\sigma$ and $\alpha \land \phi_{in} \land \overline{\phi_{out}}$, i.e., the formula in ~\autoref{eq:prop} representing the original problem  (line~\ref{line:lpsolver}).
...
In this PA you will implement symbolic execution (SE) on a given fully-connected (deep) neural network (DNN) as described in~\autoref{sec:se-smt}. You can consider DNN as a specific type of programs and thus can ``execute'' it.  Symbolic execution would execute the DNN on symbolic inputs and returns symbolic outputs, i.e., a constraint representation.  We will represent the symbolic outputs as a logical formula and use a constraint solver to check for \emph{assertions}, i.e., properties about the DNN. This assignment has two parts: (1) implement symbolic execution on a given DNN, and (2) evaluate the scalability of your implementation on various DNNs.
...
\item \textbf{DNN Encoding}: We can encode this DNN and additional details using Python:
...
\item \textbf{Constraint Solving}: We can use a constraint solver such as \texttt{Z3} to query various things about this DNN from the obtained symbolic states. Examples include: 
...
In this part you will show that symbolic execution \emph{does not} scale on large neurons.
...
\begin{enumerate}
    ...
    \item You will do this for various sizes of neurons until you feel that your computer cannot handle it anymore (e.g., until it takes about a minute or two).
...
l = [2,3,4,3,3]   # 2 inputs, 3 hidden layers (with 3,4,3 hidden neurons for respective hidden layer 1, 2, 3), and 3 outputs.
...
\begin{enumerate}
\item a main function \texttt{my\_symbolic\_execution(dnn)} that 
    \begin{itemize}
...
You can be as creative as you want, but your SE must not run for too long and must not take up too much space (e.g., do not generate over 50MB of files).  Use the \texttt{README} to tell me exactly how to compile/run your program, e.g., \texttt{python3 se.py}.
...
\begin{enumerate}
\item the zip file must be named \texttt{pa1-yourgroupname.zip} (1 submission per group)
\item One single main source file \texttt{se.py}. Indicate in the README file on how I can run it.
    \begin{itemize}
    \item your file should include at least a \texttt{my\_symbolic\_execution} function and two test functions \texttt{test} and \texttt{test2()} as indicated above
    \end{itemize}
\item a \texttt{readme.txt} file
...
The \texttt{readme.txt} file should be a \textit{plain ASCII text file} (not a Word file, not an RTF file, not an HTML file) consisting of the following:
...
\item \textbf{Answer the following questions about your SE}
    \begin{enumerate}[label=(\alph*)]
    \item Briefly describe your SE algorithm
    \item What do you think the most difficult part of this assignment?  (e.g., programming symbolic execution, using Z3, etc)
...
\subsection*{Grading (out of 20 points):}

\begin{itemize}
\item 15 points — for a complete SE as described
    \begin{itemize}
    \item 10 points if your SE works for the given example (i.e., \texttt{test()} produces similar results)
...
\item 10 points - for complete scalability analysis and plots (part 1 and part 2) as described
  \begin{itemize} 
    \item 5 points for \texttt{create\_random} code
...
% \paragraph{Exercise 5 (Verification).}  
...
\begin{algorithm}[t]
    ...
    \lIf(\tcp*[h]{orig prob (~\autoref{eq:prop}) is satisfiable}){$\isTotal(\assignment)$}{\Return {\true} }\label{line:full}
