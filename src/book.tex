...
\newcommand{\exerciseautorefname}{Exer.}
...
\begin{exercise}[Postcondition Negation]\label{ex:negation-trick}
...
\]
Each complete activation pattern corresponds to exactly one linear region of the input space.  
\end{definition}
...
It represents all complete activation patterns that extend it, and therefore corresponds to a union of multiple linear regions.  
\end{definition}
...
\]
(where $*$ denotes ``undetermined'' or ``don't care'') represents \emph{both} complete patterns  
$\langle \texttt{true}, \texttt{false}, \texttt{true} \rangle$ and  
$\langle \texttt{true}, \texttt{false}, \texttt{false} \rangle$.  
Thus $q$ compactly captures multiple linear regions at once.
\end{example}
...
Here $x_3$ and $x_4$ are ReLU neurons, and $x_5$ is a linear output.  
The activation status of the network is determined by the signs of the pre-activation values
\[
...
\begin{itemize}
  ...
  In this case, the network reduces to the linear function
  \[
  ...
  \]
  means $\hat{x}_3 \geq 0$ but places no restriction on $\hat{x}_4$.  
  Thus $q$ compactly represents two complete patterns:
  \[
  \langle \texttt{true}, \texttt{true} \rangle \quad \text{and} \quad
  \langle \texttt{true}, \texttt{false} \rangle .
  \]
  The corresponding input space is the union of the two linear regions where $x_3$ is active and $x_4$ may be either active or inactive.
\end{itemize}

This example illustrates how a complete activation pattern corresponds to a single linear region of the input space with a fixed linear function, while a partial pattern captures multiple regions simultaneously.


\paragraph{Discussion.}  
Modern DNN verification techniques~\cite{bunel2020branch,wang2021beta,ferrari2022complete,duong2024harnessing,duong2023dpllt,ovalbab,katz2019marabou,bak2021nnenum} adopt this principle by systematically searching over the space of activation patterns.  
By analyzing partial patterns, they can prune large subsets of infeasible or redundant regions without exploring each complete pattern individually, which is essential given that the number of possible complete activation patterns grows exponentially with the number of neurons.
...
\begin{exercise}[Pattern Enumeration]
Consider an NN with 3 ReLU neurons: $n_1, n_2, n_3$.
\begin{enumerate}
...
\begin{exercise}[Constraint Induction from Pattern]
Consider the toy DNN in \autoref{fig:dnn-b}. Suppose we fix the activation pattern
\[
p=\{b_3=\mathsf{true}, b_4=\mathsf{false}\}.
\]
\begin{enumerate}
  \item Write down the corresponding linear constraints on inputs $(x_1,x_2)$ implied by this pattern.
  \item Show that $p$ defines a convex polyhedron in the input space.
\end{enumerate}
...
\begin{exercise}[Satisfiability via Partial Patterns]
...
\begin{exercise}[Overlapping Regions]
