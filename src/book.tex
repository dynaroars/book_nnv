...
\begin{example}
In Z3, we can declare ReLU as a piecewise linear function and use it in assertions as follows:
...
\end{lstlisting}
\end{example}
\paragraph{Nonlinear Property} However, despite being piecewise linear, ReLU is \textbf{nonlinear} because it does not satisfy the two core properties of a linear function
...
This non-linearity makes DNN verification difficult. In fact, verification DNNs with ReLU is $\mathsf{NP}$-complete as shown in~\autoref{sec:complexity}. We will use ReLU throughout this book as the default activation function for hidden neurons in an NN.
...
A neural network answers two related questions:  
...
A NN outputs car acceleration $a$. We want: if distance to obstacle $d<10$, then $a\leq 0$. But we observe a scenario where $d=5$ but $a=+2$.  
...
\begin{exercise}\label{ex:prop-neg1}
  Let $\alpha$ represent our network and the robustness property $| x - x | \le \epsilon \implies  f(x) = f(y)$. Form the satisfiability formula (\autoref{eq:prop1}) that we need to check.
\end{exercise}


\begin{exercise}\label{ex:prop-neg2}
  Let $\alpha$ represent our network and the property $y>0$ for any input $x_1 \in [r_1,r_2], x_2 \in [r_3,r_4]$. Form the satisfiability formula (\autoref{eq:prop1}) that we need to check.
\end{exercise}

\begin{example}\label{ex:dnn-sat}
We represent the DNN in~\autoref{fig:dnn-b} as a formula $\alpha$ as follows:
\begin{align*}
...
The complete activation pattern
\[
p = b_3=\mathsf{true},\;b_4=\mathsf{false}\}
\]
fixes both neurons: $x_3$ is active  and $x_4$ is inactive.
This uniquely defines a single (complete) assignment to ReLU activation status.
%This uniquely defines one linear region of the input space.

\paragraph{Example (Partial pattern).}
If we only fix $n_1$ to be active,
\[
p' = \{b_1 = \mathsf{true}\},
\]
then $p'$ is a partial activation pattern.
...
(x \lor y) \land (\lnot x \lor z), 
\]
evaluate its truth value under all $2^3=8$ possible assignments of $x,y,z$. Identify the satisfying assignments (eval to True).
\end{exercise}
...
(0, 10);
$(\frac{80}{3} ,0)$; and 
$(\frac{280}{11},\frac{40}{11})$ (from x + 4 y = 40 ~\&~ 3x + y = 80). Note that all of these are feasible with $2x + 7y \le 800$.
