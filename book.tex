...
The affine transformation (AF) of a neuron consists of a \emph{linear combination}---i.e., a weighted sum---of its inputs, followed by the addition of a bias term. More specifically, for a neuron \(y\) with weights \(w_1, \dots, w_n\), bias \(b\), and inputs \(v_1, \dots, v_n\) from the previous layer, the affine transformation computes:
...
\begin{example}
In \autoref{fig:dnn}, neuron \(x_3\) receives inputs \(x_1\) and \(x_2\) with weights \(-0.5\), \(0.5\), and bias \(1.0\), so its affine transformation is $z = -0.5 x_1 + 0.5 x_2 + 1.0$.
\end{example}
...
Neural networks vary in architecture depending on how information flows through them and how computations are structured. Most models can be understood as variations on the \emph{feedforward neural network}, with additional structures or constraints layered on top. This section first introduces feedforward networks, then discusses common architectural variants, and finally outlines other distinct classes of neural networks.
...
\paragraph{Fully Connected Neural Networks} A \emph{fully connected} NN connects each neuron in a layer to every neuron in the next layer. Thus, every neuron in the input layer is connected to every neuron in the first hidden layer, every neuron in the first hidden layer is connected to every neuron in the second hidden layer, and so on, until the output layer. Fully connected NNs, sometimes called \emph{dense networks}, are the most basic type of feedforward network and are commonly used for tasks like classification. 

\begin{example}
\autoref{fig:dnn} earlier showed a fully connected network with two inputs and one hidden layer with two neurons, and one output neuron. \autoref{fig:ffn} below shows a fully connected network with four inputs, two hidden layers with five neurons each, and three output neurons.
    ...
    \begin{tikzpicture}[scale=0.8, transform shape]
        ...
        \foreach \i in {1,2,3,4,5}
            \foreach \j in {1,2,3}
                \draw[->] (H2\i) -- (O\j);
    \end{tikzpicture}
    \caption{A fully connected FNN with two hidden layers.}
    \label{fig:ffn}
\end{figure}
\end{example}


\paragraph{Convolutional Neural Networks (CNNs)} replace fully connected layers with \emph{convolutional layers}, which apply local filters across the input space. They are commonly used in computer vision and image processing. Despite their local structure, CNNs remain feedforward: data flows forward without cycles.
...
\paragraph{Residual Networks (ResNets)} extend feedforward networks by adding \emph{skip connections}—direct links that bypass one or more layers. This structure helps with training deeper networks by mitigating the vanishing gradient problem.

\autoref{fig:resnet}  shows an example of a residual network with skip connections.
...
\subsection{Other Neural Network Architectures}

Not all neural networks are strictly feedforward. Some architectures introduce cycles, dynamic connections, or non-Euclidean data structures.

\paragraph{Recurrent Neural Networks (RNNs)} process sequential data by introducing recurrent connections that allow information to persist across time steps. \autoref{fig:rnn} shows a simple RNN cell.
...
\autoref{tab:nn-types} summarizes several common neural network architectures and their typical application domains.

\begin{table}[ht]
\caption{Popular Neural Network Architectures and Applications}
\label{tab:nn-types}
\centering
\small
...
\paragraph{Residual Networks} Residual Networks (ResNets) are a type of neural network that is often used in image recognition and classification. 
...
\paragraph{Scalability} Neural networks can be very large, with millions of parameters. This makes it difficult to verify their correctness, as the number of possible inputs grows exponentially with the size of the network.
