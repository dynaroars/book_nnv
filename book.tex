...
In program analysis, in particular abstract interpretation, we reason about the behavior of a program without evaluating it on all concrete inputs---which may be infinite. Instead, we use an \emph{abstract domain} to summarize sets of concrete values, enabling sound and scalable approximation.

\medskip

The first key component is the \emph{abstraction function}:
\[
\alpha : D \to D^a
\]
which maps a concrete value from the domain $D$ to an element in a finite or simpler abstract domain $D^a$. For example, the \texttt{odd/even} abstraction is defined as:
\[
\alpha_{\texttt{parity}}(x) =
\begin{cases}
\texttt{even} & \text{if } x \bmod 2 = 0 \\
\texttt{odd}  & \text{if } x \bmod 2 = 1
\end{cases}
\]
Even though $\mathbb{Z}$ is infinite, this abstraction maps all integers to a finite set $\{\texttt{odd}, \texttt{even}\}$. Abstraction functions like $\alpha$ allow us to represent infinite sets compactly.

\medskip

However, once the abstract domain is defined, we do not evaluate the concrete function $f : D \to D$ directly. Instead, we define an \emph{abstract transformer function}:
\[
f^a : D^a \to D^a
\]
which operates entirely within the abstract domain and overapproximates the behavior of $f$ on all values represented by an abstract input. Specifically, given a concretization function $\gamma : D^a \to \mathcal{P}(D)$ that maps an abstract value to the set of concrete values it represents, we require:
\[
f(\gamma(x^a)) \subseteq \gamma(f^a(x^a))
\]
This is the standard \emph{soundness condition} in abstract interpretation: applying the concrete function to all values in the concretization of $x^a$ yields results that are safely covered by the concretization of $f^a(x^a)$.

\medskip

\textbf{Example 1 (Intervals).}  
Let $f(x) = x + 1$ and consider the interval abstraction $D^a = \{ [a, b] \mid a \leq b \in \mathbb{Z} \cup \{-\infty, +\infty\} \}$. Then:
\[
f^a([a, b]) = [a+1, b+1]
\]

\textbf{Example 2 (Sign abstraction).}  
