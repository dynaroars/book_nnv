...
Consider the following toy ReLU network:
\[
\begin{aligned}
\hat{x}_3 &= x_1 - 2x_2 + 1,\\
x_3 &= \relu{\hat{x}_3},\\
...
A verifier uses a BaB-style proof generation method that performs \emph{neuron splitting} on the ReLU nodes $x_3$ and $x_4$.  
Assume the verifier splits neurons in the following order:
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item Explain why the proof tree is a correctness proof.  In other words, justify why showing that all leaf nodes are \unsat{} implies that the property $y \le 3$ holds for all inputs.
\end{enumerate}
\end{problem}
...
\begin{itemize}
  ...
  \item Week 9 (Oct 22):
    ...
    \begin{itemize}        
        \item Topics: 
        \item HW: Do~\autoref{prob:proof-tree}
        \item Quiz:
    \end{itemize}
