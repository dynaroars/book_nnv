...
To encode binary variables in MILP, we typically use 0-1 integer variables. For example, let $z \in \{0,1\}$ be a binary variable indicating whether a certain condition is met.
...
\begin{itemize}
 \item If $z=1$: $x \geq 5$.  
 \item If $z=0$: $x \geq 5 - M$ (vacuous if $M$ is large).
\end{itemize}
...
\begin{example}[Encoding ReLU in MILP]
