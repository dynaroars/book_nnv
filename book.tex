...
\newcommand{\me}{ThanhVu (Vu) Nguyen and Hai Duong}
...
\chapter{Propositional Logic and Satisfiability}\label{sec:propositional-logic}
...
\begin{itemize}
  ...
  \item Negation: If $\phi$ is a formula, then so is $\overline{\phi}$.
...
\begin{itemize}
    \item \emph{Implication}: $\phi \to \psi \equiv \overline{\phi} \lor \psi$
...
\subsection{Conjunctive Normal Form (CNF)} A formula is in CNF if it is a \emph{conjunction} of  \emph{clauses}, where each clause is a \emph{disjunction} of \emph{literals}, where a literal is either a variable $p$ or its negation $\overline{p}$. 

\begin{example}
\[
(x \lor y) \land (\overline{x} \lor z)
\]
is in CNF. Each clause $(x \lor y)$, $(\overline{x} \lor z)$ is a disjunction of literals.
\end{example}
...
\section{Semantics}\label{sec:propositional-semantics}

A logical formula in propositional logic has a truth value, which can be either \texttt{True} or \texttt{False}. 
For example, the formula $x \lor \overline{x}$ is always \texttt{True}, while the formula $x \land y$ is \texttt{True} if both $x$ and $y$ are \texttt{True}, and \texttt{False} otherwise.
...
\begin{align*}
  ...
  \sigma(\overline{\phi}) = \texttt{True} &\quad \text{iff } \sigma(\phi) = \texttt{False}, \\
...
The SAT problem---checking if a propositional formula is satisfiable---is \NP-Complete~\cite{TODO}.
Thus research into methods for efficiently solving  problem instances has been ongoing for multiple decades. We discuss one of the most influential algorithms, DPLL, next.
...
Decision is the main source for branching---state space exploration---in DPLL.
The reason is that these operations use random choices or heuristics (e.g., VSIDS~\cite{moskewicz2001chaff}) to select the variable and truth value. 
Value assignments can be \emph{wrong} (e.g., $x$ should be False but is assigned True), leading to conflicts later on. 

This assignment creates a new \emph{decision level}.
...
BCP is very useful because it can deduce variable assignments directly without making decisions (guesses), which could be wrong. For example, if we have the clause $(\lnot x_1 \lor x_2)$ and we know $x_1$ is True, then we don't need to guess $x_2$; it must be True to satisfy the clause.
...
\subsection{Conflict Detection and Analysis}\label{sec:conflict-analysis}
...
\chapter{Satisfiability Modulo Theories (SMT)}\label{chap:smt}
...
\end{example}
\section{SMT Solvers}\label{sec:smt-solvers}
...
\begin{itemize}
    ...
    \item Topics: Background on Logic~\autoref{chap:propositional-logic} and Linear Programming~\autoref{chap:lp}
