...
\begin{problem}\label{problem:reluplex-hand}
    ...
    \begin{itemize}
    \item Rewrite the steps in~\autoref{ex:reluplex} on paper, showing all the calculations for each step. The goal of handwriting the steps is to help you understand how \reluplex{} works in detail.  Take pictures and submit as attachment.
...
\chapter{SAT and SMT Solving Algorithms}\label{chap:sat-smt-algorithms}

Here we illustrate the classical DPLL and CDCL algorithms to determine the satisfiability (and therefore validity) of SAT and SMT formulae.

\section{Satisfiability (SAT) Problem}\label{sec:sat-problem}
...
% =========================================================
\paragraph{Boolean Constraint Propagation (BCP)}
...
assigned True. BCP is useful because it can deduce variable assignments without making decisions.
...
A \emph{conflict} occurs when a clause becomes empty; that is, all literals in the
clause are assigned False.

\begin{example}[Immediate Conflict]
\[
