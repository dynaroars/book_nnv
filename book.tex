...
\begin{tikzpicture}[scale=#1, transform shape, semithick, ->]
    ...
    \draw (x2) -- (x4) node[midway, below] {$-0.5$} ;
    \draw (x3) -- (x5) node[midway, below] {$-2.0$};
    \draw (x4) -- (x5) node[near end, below] {$3.0$};
...
\begin{itemize}
    \item write the formula $\alpha$ representing the network. \item Then create the property that the output $x_5 \ge x_6$ for any inputs $x_1 \in [-1,1], x_2\in[-1,1]$.  \item Generate the satisfiability formula (\autoref{eq:nnv2}).  
\end{itemize}    
DO NOT use Z3 to check this formula.
\end{problem}
...
An NN has many possible---potentially infinite---inputs, making it impossible to verify, which requires checking all inputs to show no counterexample exists. However, DNNs with piecewise linear activation functions have a special structure that can be exploited to make verification feasible. 
