...
\chapter{Linear and Mixed-Integer Programming Foundations}

Linear programming (LP) is one of the most successful mathematical modeling tools of the 20th century. 
It provides a unified way to model resource allocation, scheduling, verification, and many other problems 
using only linear constraints and a linear objective function.  

In DNN verification, LP and its generalization MILP (Mixed-Integer Linear Programming) 
play a central role because neural networks with piecewise-linear activations (like ReLU) 
can be exactly represented with such constraints.

This chapter presents LP and MILP at an intuitive but rigorous level: 
geometry of feasible regions, algorithms, duality, extensions, 
and their application to neural network verification.

---

\section{Linear Constraints and Feasible Sets}

A linear inequality has the general form
\[
a_1x_1 + a_2x_2 + \cdots + a_nx_n \leq b,
\]
where $a_i, b \in \mathbb{R}$.
The set of solutions is a half-space in $\mathbb{R}^n$.
...
2x+y \leq 6
\]
means “all points $(x,y)$ lying on or below the line $2x+y=6$.”

\paragraph{Intersection of Constraints.}
The feasible set of an LP is the intersection of several half-spaces.  
Geometrically this is a \emph{polyhedron} (a polygon in 2D, polytope in higher dimensions).

\begin{center}
\begin{tikzpicture}[scale=0.7]
\draw[->] (0,0) -- (7,0) node[right] {$x$};
\draw[->] (0,0) -- (0,6) node[above] {$y$};
\draw[domain=0:6, smooth, variable=\x, thick] plot ({\x}, {(6-2*\x)});
\fill[blue!20, domain=0:3, variable=\x]
  (0,0) -- plot ({\x},{(6-2*\x)}) -- (3,0) -- cycle;
\node at (1.5,1) {feasible region};
\end{tikzpicture}
\end{center}
...
A general LP is:
\[
\max \; c^T x \quad \text{subject to } A x \leq b, \; x \in \mathbb{R}^n.
\]
...
\paragraph{Corner-Point Principle.}  
If the feasible region is bounded and non-empty, the optimum occurs at a \emph{vertex} (corner) of the polyhedron.  
This explains why algorithms like the simplex method, which move between corners, are effective.
...
- Objective values: $0,4,4$.  
- Optimum value = 4 at $(4,0)$ or $(0,4)$.

---

\section{Algorithmic View}

\subsection{The Simplex Method (idea)}

- Start at a vertex of the feasible region.  
- Move along an edge to a better vertex.  
- Repeat until no adjacent vertex improves the objective.  

Despite its exponential worst-case complexity, simplex is extremely efficient in practice.

\subsection{Interior Point Methods (idea)}

- Move through the interior of the feasible region following a central path.  
- Polynomial time guarantee, important for very large LPs.  

---

\section{Duality}

Every LP has a corresponding \emph{dual} LP.  

Primal (maximization, $\leq$ constraints):
\[
...
If both primal and dual are feasible, their optimal values coincide.  
This is a deep result and underpins many verification certificates.

\paragraph{Example.}  
Primal:
\[
\max 3x_1 + 2x_2 \quad \text{s.t. } x_1+x_2 \leq 4, \; x_1, x_2 \geq 0.
\]
Dual:
\[
\min 4y \quad \text{s.t. } y \geq 3, \; y \geq 2, \; y \geq 0.
\]
Optimal values both = 12.

---

\section{Unboundedness and Infeasibility}

\paragraph{Unbounded.}
\[
\max x \quad \text{s.t. } x \geq 0.
\]
Feasible region extends infinitely, objective grows without bound.

\paragraph{Infeasible.}
\[
x \geq 1, \quad x \leq 0.
\]
No feasible point.
...
LP assumes continuous variables.  
MILP extends LP by requiring some variables to be integers (often binary).

\paragraph{Why integers?}  
- Discrete choices (open/close a facility).  
- Logical constraints (if/then conditions).  
- Network verification: active/inactive ReLU.

\subsection{Encoding Logic with Binary Variables}

\paragraph{Big-M Trick.}
...
- If $z=1$: $x \geq 5$.  
- If $z=0$: $x \geq 5 - M$ (vacuous if $M$ is large).

\subsection{Encoding ReLU in MILP}
...
\section{Applications to DNN Verification}

Neural networks are compositions of affine transformations and nonlinearities.  
- Affine layers $\Rightarrow$ linear constraints.  
- ReLU $\Rightarrow$ MILP encoding.  
- Safety property $\Rightarrow$ extra linear constraints.  

Verification:  
\begin{itemize}
  \item Encode network + property as MILP.  
  \item Add negation of property.  
  \item Solve MILP.  
  \item If UNSAT: property holds.  
  \item If SAT: solver gives counterexample input.  
\end{itemize}
...
A \emph{linear program} (LP) is an optimization problem of the form:
\[
\begin{aligned}
...
where
\begin{itemize}
  \item $x \in \mathbb{R}^n$ are the decision variables,
  \item $c \in \mathbb{R}^n$ defines the linear objective function,
  \item $A \in \mathbb{R}^{m \times n}, b \in \mathbb{R}^m$ define the linear constraints.
\end{itemize}
...
\]
defines a half-space. The feasible region is the intersection of these half-spaces.
Thus, the feasible region is always a convex polyhedron.
...
\subsection{Vertices and Optimal Solutions}

A fundamental theorem: if the feasible region is non-empty and bounded, 
then the optimal s
