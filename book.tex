...
% \KwIn{Linear constraints $A\mathbf{x} = b$, variable bounds $\ell \le \mathbf{x} \le u$, ReLU constraints $\hat{x}_i = \max(x_i,0)$}
% \KwOut{\sat{} assignment or \unsat{}}

% \BlankLine
% \textbf{Initialization:}\;
% \Indp
% Choose a set of basic variables and rewrite $A\mathbf{x}=b$ into basic form.\;
% Initialize all variable values (e.g., 0). \;
% \Indm

% \BlankLine
% \While{true}{
%     \BlankLine
%     \textbf{1. Check bound violations.}\;
%     \If{some variable $v$ violates its bounds}{
%         \If{$v$ is non-basic}{
%             Update $v$ toward the nearest bound and propagate updates to all basic variables depending on $v$.\;
%         }
%         \Else(\tcp*[f]{basic variable}){
%             Choose a variable $u$ in $v$'s defining equation and pivot: swap $v$ with $u$.\;
%             Update the now non-basic $v$ to satisfy its bounds and propagate the changes.\;
%         }
%         \Continue\tcp*[f]{restart loop}
%     }
    
%     \BlankLine
%     \textbf{2. Check ReLU consistency.}\;
%     \If{some ReLU pair $(x_i,\hat{x}_i)$ violates $\hat{x}_i=\max(x_i,0)$}{
%         Let $v$ be the violated variable ($x_i$ or $\hat{x}_i$). \;
%         \If{$v$ is non-basic}{
%             Update $v$ to satisfy the ReLU relation and propagate changes.\;
%         }
%         \Else{
%             Pivot $v$ with a variable in its defining equation.\;
%             Update the new non-basic variable to restore consistency.\;
%         }
%         \Continue
%     }
    
%     \BlankLine
%     \textbf{3. All constraints satisfied.}\;
...
Thus, in this example, we conclude that property in~\autoref{eq:prop1} is \emph{not valid} for the DNN in~\autoref{fig:dnn-replux} because for the inputs $x_1=0,x_2=-1$, the DNN gives the output $x_5=0.5$, which violates the property in~\autoref{eq:prop1}.
\end{example}
