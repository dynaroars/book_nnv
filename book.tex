...
\begin{problem}
    Suppose $y$ can take values from 0 to 3. Student A computes an interval abstraction for $y$ as $y \in [-2,4]$, while Student B computes the interval abstraction as $y \in [1,2]$. 
    \begin{enumerate}
        \item Which student's abstraction is correct?  Explain your answer. If both are correct, which are more precise?
        \item Using the correct abstraction (if both are correct, use the more precise one), can we verify the property $y \leq 3$? How about $y \le 5$? Explain your answers.
    \end{enumerate}
\end{problem}
\subsection{Zonotope}\label{sec:zonotope-abstraction}

An interval abstraction (\autoref{sec:interval-abstraction}) treats each variable independently. For example, if \(x_1 \in [1,2]\) and \(x_2 \in [3,4]\), intervals assume any combination of \(x_1\) and \(x_2\) is possible. But variables can correlate: e.g., when \(x_1\) increases, \(x_2\) also increase.  Thus, zonotopes, which can capture such correlations using their generators, can provide a tighter abstraction.
...
\end{tikzpicture}
\caption{Why a zonotope can become a parallelogram.  
...
\paragraph{Step 2: Apply the affine transformation} We apply the affine function \(f(x_1, x_2) = -0.5\,x_1 + 0.5\,x_2 + 1.0\),  
...
\paragraph{Step 3: Compute output bounds} From the output zonotope, we can compute the lower and upperbounds for \(x_3\) by subtracting and adding the absolute values of the generator coefficients:
...
For \(\text{ReLU}(x) = \max(0, x)\), the shape of the input region changes at \(x = 0\).  
...
\textbf{Step 1: Determine the bounds of the input zonotope} As shown in~\autoref{ex:zonotope_linear_example}, the output zonotope (before ReLU) has bounds \([-0.5, 2.0]\).   So the input zonotope to ReLU has bounds \([-0.5, 2.0]\). Since \(l = -0.5 < 0 < 2.0 = u\), this is an unstable neuron requiring over-approximation.

\textbf{Step 2: Slope approximation}
We use slope 

\[
\lambda = \frac{u}{u-l} = \frac{2.0}{2.0-(-0.5)} = \frac{2.0}{2.5} = 0.8.
\]

\textbf{Step 3: Transform zonotope}
...
\textbf{Step 4: New generator for approximation error}
...
An \emph{empty} activation pattern assigns \emph{no} activation statuses to any neurons:
\[
p = \langle *, *, \dots, * \rangle \in \{\texttt{true},\texttt{false},*\}^m .
\]
In other words, it represents \emph{all} possible complete activation patterns.
\end{definition}
