...
A zonotope is a geometric object that generalizes the idea of an interval, which is a zonotope in 1D (\autoref{ex:interval-zonotope}), to multiple dimensions.
A zonotope is represented using a center point and several generator vectors. More specifically, a zonotope is built by starting from a center point and then stretching along several directions (generators).
...
\]
where \(g_j\) are the generator vectors,  
the abstract transformer \(f^a\) for the affine function is:
\[
...
%\mathcal{Z} = \left\{ c + \sum_{i=1}^{m} \epsilon_i g_i \mid \epsilon_i \in [-1, 1] \right\}
\paragraph{Step 1: Represent the input as a zonotope.}  
We represent the input intervals as a zonotope $\{ c + \sum_{i=1}^{m} \epsilon_i g_i \mid \epsilon_i \in [-1,1] \}$.
...
\paragraph{Step 2: Apply the affine transformation.}  
Using the affine function \(f(x_1, x_2) = -0.5\,x_1 + 0.5\,x_2 + 1.0\),  
we apply it to $\mathcal{Z}$:

\begin{equation}
    \small
\begin{aligned}
f^a(\mathcal{Z})
  &= f(c_1, c_2) + \epsilon_1 f(g_1) + \epsilon_2 f(g_2) \\
  &= [-0.5(1.5) + 0.5(1.0) + 1.0] + \epsilon_1[-0.5(0.5) + 0.5(0)] + \epsilon_2[-0.5(0) + 0.5(2.0)]\\
    &= 0.75 + \epsilon_1(-0.25) + \epsilon_2(1.0).\\
\end{aligned}
\end{equation}
...
\paragraph{Step 3: Compute the output bounds.}  
The lower and upper bounds are obtained by subtracting and adding the absolute values of the generator coefficients:
\[
\begin{aligned}
...
Note that for this simpl example, both interval (\autoref{sec:interval-affine-transformer}) and zonotope abstractions yield identical bounds \([-0.5, 2.0]\).  
...
In this chapter, we discuss work~\cite{duong2025generating} on generating and checking proofs of \unsat{} results for DNN verification.
