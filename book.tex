...
Due to scalability issues with SMT solvers (\autoref{sec:smt-limitations}), modern DNN verification techniques often rely on Mixed Integer Linear Programming (MILP) solving, which is specialized for linear constraints and is more efficient for DNN reasoning. Many state-of-the-art DNN verification tools~\cite{wang2018efficient,tjeng2019evaluating,duong2024harnessing} use the industrial-strength MILP solver Gurobi~\cite{gurobi}.
...
These constraints enforce $\hat{z} = z$ when $a = 1$ (active, $z \geq 0$) and $\hat{z} = 0$ when $a = 0$ (inactive, $z < 0$). This encoding allows the MILP solver to reason precisely about the nonlinearity using only linear constraints and binary (integer) variables. If the solver sets $a = 1$, it means that the neuron is active, and $\hat{z} = z$.
If the solver sets $a = 0$, it means that the neuron is inactive, and $\hat{z} = 0$.

Note the importance of the upper and lower bounds $u$ and $l$ on $z$. Without these bounds, the MILP encoding can become unsound or too loose. For example, without $u$, then the constraint $\hat{z} \leq a \cdot u$ cannot properly limit $\hat{z}$ when $a = 1$, potentially allowing infeasible values for $\hat{z}$. Similarly, if $l = 0$, then the constraint $\hat{z} \leq z - l(1 - a)$ becomes $\hat{z} \leq z$, which does not prevent $\hat{z} > 0$ when $a = 0$ and $z$ is slightly positive—thus violating ReLU semantics. Providing tight bounds ensures the MILP solver correctly captures the behavior of $\hat{z} = \max(z, 0)$ in all cases.
...
\begin{example}
    ...
    We have $x_3 = -0.5x_1 + 0.5x_2 + 1.0$---the pre-activation value of $x_3$. The upper $u_3$ and lower $l_3$ bounds on $z_3$ over the input region $x_1 \in [-1,1]$ and $x_2\in[-2,2]$ are:
\begin{align*}
    & z_3 = -0.5x_1 + 0.5x_2 + 1.0 \\
    & l_3 = -0.5(-1) + 0.5(-2) + 1.0 = 0.0 \\
    & u_3 = -0.5(1) + 0.5(2) + 1.0 = 1.0 \\
\end{align*}
...
\implies  &\hat{x}_3 \leq a_3 \cdot 1.0 \\
    &\hat{x}_3 \leq x_3 - l_3(1-a_3) \\
\implies  &\hat{x}_3 \leq x_3 - 0.0(1-a_3)\\ 
\implies  &\hat{x}_3 \leq x_3 \\
    &a_3 \in \{0,1\}
\end{align*}


These constraints encode $\hat{x}_3 = \max(x_3, 0)$. 
The first constraint asserts that the output $\hat{x}_3$ is at least as large as the pre-activation value $x_3$, and the second constraint asserts that $\hat{x}_3$ is non-negative. The third constraint limits $\hat{x}_3$ to be at most $u_3$ when $a_3 = 1$, and the fourth constraint ensures that if $a_3 = 0$, then $\hat{x}_3$ is zero, as it simplifies to $\hat{x}_3 \leq x_3$ when $l_3 = 0$.

This encoding allows the MILP solver to reason about the activation of neuron $x_3$ using linear constraints and the binary variable $a_3$. If the solver sets $a_3 = 1$, it means that neuron $x_3$ is active, and $\hat{x}_3 = x_3$. If it sets $a_3 = 0$, then neuron $x_3$ is inactive, and $\hat{x}_3 = 0$.

\end{example}    
\subsection{DNN encoding} The general MILP encoding of a DNN is then:

%\tvn{unclear, SMT solver works the same way too? they also use DPLL(T)}
...
%We can encode the DNN as a set of linear constraints and use an MILP solver to check that the constraints are satisfiable. 
...
\begin{example}\label{ex:milp-dnn}
    We create the following MILP formulation for the example in~\autoref{ex:smt-dnn}. Recall the DNN \autoref{fig:dnn-a} in the example has two inputs $x_1, x_2$, 1 hidden layer with 2 neurons $x_3, x_4$, each with its own ReLU output $\hat{x}_3, \hat{x}_4$, and 1 output neuron $x_5$.
...
\item Computing upper and lower bounds
The bounds $l_3, u_3, l_4, u_4$ can be computed over the input ranges. For instance, with $x_1 \in [-1,1]$ and $x_2 \in [-2,2]$:
\begin{align*}
