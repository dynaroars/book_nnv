...
\begin{example}
For~\autoref{ex:transformer-affine1}, applying ReLU to the output bounds of neuron $x^3$ gives:
...
This overapproximation grows quickly as we propagate through many layers of a large network, i.e., it keeps ``inflating'' the bounds, leading to a very loose approximation of the output and becomes useless for verification tasks. For example, $z \leq 0$ is valid for~\autoref{ex:interval-overapproximation}, but the interval abstractions gives $z \in [0,1]$, which is not tight (precise) enough to verify this property.
