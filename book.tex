...
\autoref{alg:bab-proof} extends~\autoref{alg:bab} to show how to generate proofs for BaB algorithms. The key idea is to introducing a proof tree (\texttt{proof}) data structure (\autoref{line:prooftree})and record the branching decisions made by the algorithm to the proof tree (\autoref{line:record_proof}). The proof tree is a binary tree structure, where each node represents a neuron and its left and right edges represent its activation decision (active or inactive). 
