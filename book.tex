    ...
    \While(\tcp*[h]{main loop}){$\problems$}{\label{line:babstart}
        ...
        % \Parfor(\tcp*[h]{process in parallel}){$\sigma_i ~\In~ \problems$}{ \label{line:parfor}
            \If{\Deduce{$\mathcal{N}, \phi_{in}, \phi_{out}, \sigma_i$}}{\label{line:babdeduce}
                ...
                $\problems \cup \problems \cup \{ \sigma_i \cup \{v_i\} ~;~ \sigma_i \land \{\overline{v_i}\} \}$
            }
...
\bab{} maintains a set of activation patterns (\texttt{ActPatterns}) that represent the current activation pattern of the network. Initially, \texttt{ActPatterns} is initialized with an empty activation pattern (\autoref{line:babinit}).
In each BaB iteration $i$ (\autoref{line:babstart}), the algorithm selects and removes an activation pattern $\sigma_i$ from \texttt{ActPatterns} (\autoref{line:babselect}).  

It then calls \functiontextformat{Deduce} to determine if the current problem, i.e., the original satisfiability problem with  activation pattern $\sigma_i$,
is feasible. \functiontextformat{Deduce} compute upper and lower bounds, e.g., using interval abstraction, of the output values using $\sigma_i$, e.g., by replacing ReLU functions according to the activation statuses specified in $\sigma_i$ (\autoref{sec:pattern-reduction}). 
If the computed bounds indicate that no counterexample can exist, e.g., the lower bound of the output is greater than 0 when checking $y < 0$, then the problem is infeasible. Otherwise, the problem is \emph{potentially} feasible (because the bounds are over-approximations).
...
If \functiontextformat{Deduce} determines that the problem is infeasible, \bab{} discards the current processing activation pattern and loops back (to~\autoref{line:babstart}) to process the next activation pattern.
\bab{} terminates when there are no more activation patterns to process, at which point it returns \unsat{} (\autoref{line:babunsat}), indicating that the property is valid.


If \functiontextformat{Deduce} determines that the problem is feasible, it calls \functiontextformat{LP} to check the satisfiability of the problem using a linear programming (LP) solver (\autoref{line:bablp}). If the LP solver finds a satisfying assignment, it returns a counterexample (\autoref{line:babsat}), indicating that the property is invalid.
Otherwise (\functiontextformat{LP} returns unsatisfiable), \bab{} calls \functiontextformat{Decide} to select a neuron $v_i$ to split, i.e., to fix its activation status as either active or inactive (\autoref{line:babdecide}).  This essentially means the problem is split into two independent subproblems: one with $v_i$ (active) and the other with $\overline{v_i}$ (inactive). \bab{} then adds the two new activation patterns $\sigma_i \land v_i$ and $\sigma_i \land \overline{v_i}$ to \texttt{ActPatterns}. \bab{} then loops back to~\autoref{line:babstart} to process the next activation pattern.
...
Equivalently, we want to show that no counterexample exists, i.e., no inputs $(x_1,x_2)$ in the given input range such that $y_1 \le y_2$.


First, \bab{} initializes \functiontextformat{ActPatterns} with an empty activation pattern $\emptyset$ (i.e., no neurons fixed). Then \bab{} enters its main loop. In the first iteration, \bab{} selects the only available activation pattern $\emptyset$, i.e., $\sigma_i = \emptyset$,
and  calls \functiontextformat{Deduce} to check the feasibility of the problem based $\sigma_i$.

Here, \functiontextformat{Deduce} determines that the problem is feasible, e.g., the bounds of $y_1$ and $y_2$ indicate that $y_1 \le y_2$ can be satisfied. Thus, \bab{} calls \functiontextformat{LP} to check the satisfiability of the problem using an LP solver. The LP solver returns unsatisfiable (or unknown)--meaning no counterexample is found in this activation pattern.  \bab{} then randomly selects a neuron to split (\functiontextformat{Decide}). Assume that it chooses $v_4$ to split, i.e., the problem spawns two independent subproblems: one with $v_4$ active and the other with $v_4$ inactive.
It does so by unioning $\{v_4\}$ and $\{\overline{v_4}\}$ with $\sigma_i$ and adds both new activation patterns to \functiontextformat{ActPatterns}. Next, \bab{} loops back to process the next activation pattern.


In the second iteration, \bab{} has two subproblems. For the subproblem with activation pattern $\{v_4\}$, \functiontextformat{Deduce} decides feasibility but \functiontextformat{LP} returns unsatisfiable (or unknown), 
so it selects $v_2$ to split by adding $\{v_4, v_2\}$ and $\{v_4, \overline{v_2}\}$ to \texttt{ActPatterns}. 
For the other subproblem with $\overline{v_4}$ inactive, \functiontextformat{Deduce} determines infeasibility and thus discards this subproblem.

In the third iteration, \bab{} has two subproblems for $v_4 \land v_2$ and $v_4 \land \overline{v_2}$. For the first subproblem, \bab{} selects $v_1$ to split by unioning $v_1$ and then $\overline{v_1}$ to the current activation pattern $v_4 \land$ and adds them to \functiontextformat{ActPatterns}. For the second subproblem, \functiontextformat{Deduce} determines infeasibility and \bab{} discards this subproblem.

In the fourth iteration, \bab{} has two subproblems: $v_4 \land v_2 \land v_1$ and $v_4 \land v_2 \land \overline{v_1}$, both which are then determined infeasible and discarded.
...
Do~\autoref{ex:bab} but come up with concrete values for each step, e.g., what are the bounds computed by \functiontextformat{Deduce}, what does \functiontextformat{LP} return, etc to illustrate the process in more detail.
\end{problem}


\begin{problem}\label{prob:bab-counterexample}
Do~\autoref{ex:bab} but assume the property is invalid and find a counterexample.
\end{problem}
...
Apply BaB on the DNN in \autoref{fig:dnn-bab} to verify the property $x_5 > 0$ for any inputs $x_1 \in [-1,1], x_2\in[-2,2]$.  Use interval abstraction to compute bounds. For LP solving, you can use any LP solver or solve it manually. Show all stepts, e.g., iterations, activation patterns, bounds computed, LP results, etc.
...
\begin{enumerate}
  \item In \autoref{alg:bab}, what happens if \functiontextformat{Deduce} always returns infeasible?
  \item What happens if \functiontextformat{Deduce} always returns feasible but \functiontextformat{LP} always returns satisfiable?
...
\subsection{Beyond the Basic} What described above is the basic and minimal BaB algorithm. Modern DNN verifiers implement many optimizations and strategies to improve the performance of the search. For example, they apply various engineering tricks to eliminate easy cases or find easy counterexamples (\autoref{chap:common-engineering}) before running the full BaB search. 
...
\tool{} returns \unsat{} if it verifies all subproblems and \sat{} if it found a counterexample in any subproblem. Modern DNN verification tools such as \neuralsat{} uses the number of available CPU threads to determine the number of created subproblems.
...
\section{Adversarial Attacks (\autoref{chap:adversarial-attacks})} Most DNN verification tools implement \emph{adversarial attack} techniques to find counterexamples before running BaB. These techniques are often efficient and can quickly find counterexamples for invalid properties.
...
If an attack algorithm gives a valid counterexample, the verifier returns \sat{}, indicating that property is invalid. Otherwise, if all attack algorithms fail to find a counterexample or they exceed a predefined timeout, the verifier proceeds to run the full BaB search.
...
Many DNN verification tools first run an adversarial attack technique to check for obvious counterexamples. If one is found, the property is violated (and the verifier returns \sat). If no counterexample is found, the verifier proceeds to the more expensive search (e.g., \bab{} as shown in~\autoref{chap:bab}).

Thus, modern verification tools have two phases: (i) attempt to \emph{falsify} the property with adversarial attacks, and (ii) if no counterexample is found, attempt to \emph{verify} the property. Of course, during the verification phase, the verifier may also discover counterexamples that the falsify phase misses.

\section{Random Search Attack}
Random search is a simple method for adversarial attack.  It randomly samples points in the allowed input ranges and checks if any samples violate the property; if so, a counterexample is found.
...
\begin{enumerate}
    ...
    (x_1^{(0)}, x_2^{(0)}) = (0, 0)
    \]
    This point is within the allowed input ranges, but does not violate the property $y > 0$ since $y(0,0) = 1 > 0$. So we need to continue.

    \item \textbf{Iterative update.}  
    For each step $t = 0, 1, 2, \ldots$:
    \begin{enumerate}
        \item \textbf{Compute the gradient.}  
        The gradient $\nabla_x y$ tells us how sensitive the output $y$ is to each input dimension:
        \[
        ...
        \nabla_x y = (2, -1.5)
        \]
        This is computed by taking the partial derivatives:
        \[
        ...
        \item \textbf{Gradient update (take a step).}  
        ...
        If the new values go outside the legal range, bring them back to the closest valid boundary:
        \[
...
Now let the step size be larger, $\eta = 1.0$, starting again from $(x_1^{(0)}, x_2^{(0)}) = (0, 0)$:
\[
\begin{aligned}
...
\end{aligned}
\]
Here, $x_1^{(1)} = -2.0$ is outside the allowed range $[-1, 1]$, so we must \emph{clip} it:
\[
...
\]
PGD again finds a counterexample—this time, the projection step ensured we stayed within bounds.

\end{example}


\begin{example}[Computing the Gradient]
Consider
\[
y = x_1^2 + 3x_2.
\]
...
For instance, at the point $(x_1, x_2) = (1, 2)$,
\[
\nabla_x y = (2 \times 1,\ 3) = (2, 3).
\]
This means that near $(1,2)$, increasing $x_1$ by a small amount increases $y$ roughly twice as fast as that same increase in $x_2$ would.
\end{example}
    ...
    \While(\tcp*[h]{main loop}){$\problems$}{
        ...
        % \Parfor(\tcp*[h]{process in parallel}){$\sigma_i ~\In~ \problems$}{ \label{line:parfor}
            \If{\Deduce{$\mathcal{N}, \phi_{in}, \phi_{out}, \sigma_i$}}{
                $(\counterexample, v_i) \leftarrow \Decide(\mathcal{N}, \phi_{in}, \phi_{out}, \sigma_i)$ \\ 
                \If(\tcp*[h]{found a valid cex}){$\counterexample$}{
...
\begin{itemize}
  ...
  \item Week 9 (Oct 22):
    ...
    \begin{itemize}        
        ...
        \item Topics: Zonotope~\autoref{sec:zonotope-abstraction} and Branch -and-Bound DNN verification~\autoref{chap:bab}
        \item Quiz: In class
    \end{itemize}
    \item Week 11 (Nov 5):
    \begin{itemize}        
