...
\begin{problem}
As shown in~\autoref{ex:relu_milp}, the upper and lowerbounds of $x_3$ are $u_3 = 2.5$ and $l_3 = -0.5$. 
\begin{itemize}
\item Compute the upper and lower bounds of $x_4$ in the same example.
\item Change the weights and/or biases of the DNN in~\autoref{fig:dnn-c} such that $x_3$ becomes a stable neuron (always active or always inactive).  Show the new weights and/or biases, and compute the new upper and lower bounds of $x_3$.
\end{itemize}
\end{problem}
...
Given the input in zonotope form \((c, G)\), $x = c + \sum_{i=1}^m \epsilon_i g_i$ where $\epsilon_i \in [-1,1]$, how can we compute the output zonotope $y = c' + \sum_{i=1}^m \epsilon_i g'_i$ where $y = \max(0, x)$?

\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{zonotope_bounds.png}
    \caption{Lower and upper bounds computation for a zonotope input.}
\end{figure}
...
Merging the two lower and upper constraints, we get the following equation:
\begin{align*}
    \lambda x \leq \quad &y \quad \leq \lambda x - \lambda l \\
    \Leftrightarrow \quad &y = \lambda x - \alpha \lambda l, \quad \alpha \in [0, 1]
\end{align*}
However, the form of zonotope requires the $\epsilon_i \in [-1,1]$, so we define $\alpha$ as:
\[
\alpha = \frac{1 + \epsilon_{new}}{2}, \quad \epsilon_{new} \in [-1, 1]
\]
% where $\epsilon_{new}$ is the new generator for the approximation error. 
The output zonotope is then:
\begin{align*}
    y &= \lambda x - \frac{1 + \epsilon_{new}}{2} \lambda l \\
...
\textbf{Final center:}
\[
c' = \lambda c - \frac{\lambda l }{2}
\]

\textbf{Final generator matrix:}
\[
G' = \left[\lambda g_1, \lambda g_2, \ldots, \lambda g_m, -\frac{\lambda l }{2}\right]
\]
}
