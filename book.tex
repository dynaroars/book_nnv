...
To create a logical formula representing the DNN in \autoref{fig:dnn-a}, we can symbolically execute the DNN on symbolic inputs $x_1,x_2$ and track the values of the neurons $x_3, x_4, x_5$ as a set (conjunction) of logical formulae. The symbolic execution starts with the inputs $x_1$ and $x_2$ and computes the values of the neurons in the hidden layer, $x_3$ and $x_4$, using the affine transformations, followed by ReLUs. Finally, it computes the output neuron $x_5$ as a linear combination of the hidden layer neurons.
...
We can then use an SMT solver to check that the property $x_5 \le 0$ is satisfiable for any inputs $x_1 \in [-1,1], x_2\in[-2,2]$ (\autoref{ex:dnn}). Specifically, this property can be expressed as a formula $\phi$:
\begin{align*}
(x_1 \ge -1) \land (x_1 \le 1) \land (x_2 \ge -2) \land (x_2 \le 2) \quad \implies (x_5 < 0)
\end{align*}
