...
\neuralsat{}~\cite{duong2025neuralsat,duong2024harnessing} is a relatively late competitor in the DNN verification space, but it has quickly become a strong contender. It also uses the BnB search strategy, but it is based on the DPLL(T) framework~\cite{davis1962machine} combined with several unique optimizations and heuristics to improve the search performance.
...
\section{Common Engineerings and Optimizations}\label{sec:common-engineering}

Modern, high-performance DNN verifiers implement several optimizations and engineering techniques to improve their performance.  Some of these are engineering tricks to deal with special cases, such as small inputs, while others are general optimizations to improve the search performance.


\subsection{Input Splitting} Like some other verifiers~\cite{katz2019marabou,katz2022reluplex,bak2021nnenum},
...
\subsection{Multiprocessing} \label{sec:input_split}
...
\subsection{Adversarial Attacks (\autoref{chap:adversarial-attacks})} Like other DNN verifiers~\cite{ferrari2022complete,zhang2022general}, \tool{} tool implements a fast-path optimization that attempts to disprove or falsify the property before running DPLL(T).
...
Although restarting may seem like an engineering aspect, it plays a crucial role in stochastic algorithms like \tool{} and helps reduce verification time for challenging problems as shown in \S\ref{sec:rq1}.
...
\section{\tool{} vs. BnB}\label{sec:restart-tree}
