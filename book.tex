...
\autoref{fig:relu-all-abstractions} illustrates common abstractions (or \emph{over-approximations}) for the ReLU function $y = \max(0, x)$, where $x \in [l_x, u_x]$ and $y \in [l_y(x), u_y(x)]$. The values of ReLU are shown as points on the \colorbox{red}{red line}, which is non-convex and consists of two linear segments: one for $x < 0$ (where $y = 0$) and another for $x \geq 0$ (where $y = x$).
...
\begin{itemize}
    ...
    In~\autoref{fig:relu-all-abstractions}(a), the \colorbox{yellow}{yellow rectangle} represents the interval $[0, u_x]$.  As can be seen, this interval region is too large of an over-approximation (too coarse or loose), as it includes many points that are not achievable by ReLU, e.g., the point $(0.5, 0.0)$ is not on the red line.

    \item \textbf{Zonotope Abstraction:} Zonotopes can capture linear relationships between variables more effectively. We can represent ReLU as a zonotope that includes the linear segments. In \autoref{fig:relu-all-abstractions}(b), the zonotope is shown as a parallelogram that captures the linear relationship between $x$ and $y$. The lower bound is \tvn{Hai?} and the upper bound is \tvn{Hai?}.

    The \colorbox{yellow}{zonotope} shown in \autoref{fig:relu-all-abstractions}(b) is arguably tighter than the interval abstraction in~\autoref{fig:relu-all-abstractions}(a).  It captures the linear relationship between $x$ and $y$ and excludes points that are not achievable by ReLU, e.g., the point $(0.5, 0.0)$ that was included in the interval abstraction is not included in the zonotope. However, it still includes non-ReLU points and is also not strictly better than interval, e.g., the point $(0.5, -0.5)$ is included in the zonotope but not in the interval abstraction (or ReLU).
    ...
    In \autoref{fig:relu-all-abstractions}(c), the polytope is shown as a trapezoid that captures the linear segments of ReLU. The lower bound is $l_y(x) = 0$ and the upper bound is $u_y(x) = u_x$. \tvn{this is similar to the interval abstraction, as both have the same bounds, right?}
...
\paragraph{Transformer Functions}

In program analysis, \emph{abstraction} is the process of mapping concrete values or computations into a simpler domain that preserves certain properties of interest. This allows us to reason about the behavior of programs in a scalable way.

To abstract or compute an overapproximation of a function $f$, we define a corresponding \emph{transformer function} $f^a$ that operates over an abstract domain. Specifically, $f^a$ maps abstract inputs to abstract outputs such that the concrete behavior of $f$ is soundly overapproximated. That is, if $x^a$ abstracts a set of concrete inputs $x$, then $f^a(x^a)$ abstracts the set $\{ f(x) \mid x \in \gamma(x^a) \}$, where $\gamma$ is the concretization function.

A simple and popular example is the interval abstraction. Consider a concrete function $f(x) = x + 1$. Its transformer $f^a$ over intervals maps $[a, b]$ to $[a+1, b+1]$.

Another example is sign abstraction. For the same function $f(x) = x + 1$, the transformer over the sign domain would be:
\[
f^a(\text{pos}) = \text{pos}, \quad f^a(\text{zero}) = \text{pos}, \quad f^a(\text{neg}) = \{\text{neg}, \text{zero}, \text{pos}\}
\]
where each element denotes an abstract sign (positive, zero, negative), and the transformer may be nondeterministic to preserve soundness.

Transformers are key components in abstract interpretation, enabling static analyses such as range analysis, null-pointer checking, and symbolic execution.


In the context of DNN verification, the transformer function $f^a$ takes as input the lower and upper bounds of the input neurons and returns the lower and upper bounds of the output neuron after applying the function $f$. Formally, for a function $f$ that takes $n$ inputs, the transformer function $f^a$ can be defined as:
\[f^a([l_1, u_1], [l_2, u_2], \ldots, [l_n, u_n]) = [l_f, u_f]\]
where $[l_f, u_f]$ are the lower and upper bounds of the output neuron after applying the function $f$ to the inputs. 
For example, for the ReLU function, the transformer function can be defined as:
\[f^a([l_x, u_x]) = [l_y(x), u_y(x)]\]
where $l_y(x) = 0$ and $u_y(x) = u_x$.

\section{Interval}\label{sec:interval-abstraction}

Interval is a simple and widely-used abstraction technique in DNN verification. It estimates the over-approximated range of values that a neuron can take using a lower and upper bound $[l, u]$. An interval $[l, u]$ denotes the potential set.
$$\{x \ | \ l \leq x \leq u\}$$


\subsection{Affine functions}

For affine function \(f\) in~\autoref{sec:affine}
\[f(v_1, v_2, ...,v_n) = \sum_{i}^{n}w_iv_i\]
where $n$ is number of output nodes from the previous layer. We can define the abstract transformer \(f^a\) for interval as follows:
