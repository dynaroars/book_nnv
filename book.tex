...
\subsection{Weights and Layers} The network structures described earlier can be generalized by allowing each node (except input nodes) to have \textbf{weights} and \textbf{biases}. With these parameters, each node computes its output by first applying an \textbf{affine transformation} to its inputs (a weighted sum plus a bias), and then applying a \textbf{non-linear activation} such as ReLU.

In most neural networks, nodes are grouped into \textbf{layers}, where each layer consists of multiple nodes that compute their outputs in parallel using the outputs from the previous layer as their inputs. The first layer that processes input data is called the \textbf{input layer}, the last layer producing the network's final outputs is called the \textbf{output layer}, and all layers in between are called \textbf{hidden layers}. The computation proceeds layer by layer, from inputs to outputs, forming a feedforward flow of information.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{\label{fig:dnn}A simple DNN with weights, biases, and ReLU activations.}
\end{figure}

\begin{example}\label{ex:dnn}
\Cref{fig:dnn} shows a simple DNN with two inputs \(x_1, x_2\), one hidden layer with two neurons \(x_3, x_4\), and one output node \(x_5\). The weights are shown on the edges, and the biases are displayed below each neuron.

\begin{itemize}
    \item The hidden neurons compute:
    \[
    x_3 = \relu{-0.5 x_1 + 0.5 x_2 + 1.0},
    \]
    \[
    x_4 = \relu{1.0 x_1 + 1.0 x_2 - 1.0}.
    \]
...
\autoref{tab:activation} summarizes the most common activation functions used in DNNs, their equations, output ranges, and key uses.
