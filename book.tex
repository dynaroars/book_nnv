...
An interval abstraction (\autoref{sec:interval-abstraction}) treats each variable independently. For example, if \(x_1 \in [1,2]\) and \(x_2 \in [3,4]\), intervals assume any combination of \(x_1\) and \(x_2\) is possible. But in many real systems, variables move together: when \(x_1\) increases, \(x_2\) might also increase.  
...
Thus, zonotopes extend intervals by introducing \emph{generators}, which are vectors that define directions of variability and allow the zonotope to tilt and stretch in directions that capture correlations between variables. 
...
\paragraph{Definition}
    A zonotope is built by taking a ``center point'' and adding several small ``generator'' directions that can each vary a bit in both directions. More specifically, a \textbf{zonotope} \(\mathcal{Z}\) in \(\mathbb{R}^n\) is defined as:
\[
...
% axes
\draw[->, thick] (-0.5,0) -- (3.5,0) node[right] {$x_1$};
\draw[->, thick] (0,-0.5) -- (0,3.0) node[above] {$x_2$};

% center point
\filldraw[blue!60!black] (1.5,1.0) circle (1.5pt) node[below left] {$c$};

% generator vectors
\draw[->, thick, blue!70!black] (1.5,1.0) -- (2.7,1.0) node[right] {$g_1$};
\draw[->, thick, blue!70!black] (1.5,1.0) -- (1.8,2.2) node[above] {$g_2$};

% parallelogram (zonotope region)
\fill[orange!20] (0.3,0.0) -- (2.7,0.0) -- (3.0,2.2) -- (0.6,2.2) -- cycle;
\draw[orange!70!black, thick] (0.3,0.0) -- (2.7,0.0) -- (3.0,2.2) -- (0.6,2.2) -- cycle;

\node[orange!70!black] at (2.1,1.6) {\small all $\epsilon_1,\epsilon_2$ combinations};

% example dots
\fill[orange!70!black] (0.3,0.0) circle (1pt);
\fill[orange!70!black] (2.7,0.0) circle (1pt);
\fill[orange!70!black] (0.6,2.2) circle (1pt);
\fill[orange!70!black] (3.0,2.2) circle (1pt);

\end{tikzpicture}
\caption{With two generators $g_1$ and $g_2$, varying $\epsilon_1,\epsilon_2\in[-1,1]$ independently fills a 2D region (a parallelogram). The four corners correspond to the extreme combinations $(\epsilon_1,\epsilon_2)\in\{-1,+1\}^2$.}
\label{fig:zonotope-e1e2}
\end{figure}
...
A key benefit of zonotopes is that affine operations preserve their structure exactly: no approximation needed.  
...
\end{equation}
    ...
    Note that, for this single affine operation over a box input, both interval (\autoref{sec:interval-affine-transformer}) and zonotope abstractions yield identical bounds \([-0.5, 2.0]\). The advantage of zonotopes becomes apparent when propagating through multiple layers, where zonotopes preserve linear correlations that intervals lose.
\end{example}

\subsubsection{ReLU Transformer}

Activation functions like ReLU are non-affine and do not preserve zonotope structure\tvn{what is it? can we draw it}.  

\paragraph{Intuition first.}
The ReLU function “clips’’ all negative values to 0 and keeps positives unchanged:
\[
\text{ReLU}(x) = \max(0, x)
\]
Graphically, it looks like a straight line \(y=x\) for \(x>0\) and flat at 0 for \(x<0\).  
If your input range \([l,u]\) is entirely above 0, everything passes through unchanged.  
If it’s entirely below 0, everything becomes 0.  
The hard case is when the range crosses 0 — the function bends sharply there, so a straight-line (affine) abstraction can’t capture it exactly.

% \begin{center}
% \includegraphics[width=0.55\textwidth]{figures/relu_parallelogram.pdf}
% \end{center}
% \textit{Figure: The ReLU curve (black) and its zonotope over-approximation (shaded parallelogram).}

\paragraph{Why not just use intervals?}
Intervals can easily over-approximate ReLU, but they lose dependencies — if two neurons share the same uncertainty source, intervals can’t remember that after the nonlinearity. Zonotopes can, thanks to their shared generators.  
This makes zonotopes much tighter when you propagate through several layers.

---

Given an input zonotope, we first compute its interval bounds \([l,u]\) (\tvn{like what we just computed in 5.2.2.1?}), then consider three cases:
...

(add rest of your existing technical section unchanged)
...
\draw[->, thick] (0,-0.5) -- (0,2.8) node[above] {$y=\text{ReLU}(x)$};

% ReLU curve
\draw[thick, black, domain=-1.2:0] plot (\x,0);
\draw[thick, black, domain=0:2.5] plot (\x,\x);

% Points
\fill (0,0) circle (1.5pt) node[below left] {\small $0$};
\fill (2,2) circle (1.5pt) node[right] {\small $(u,u)$};
\fill (-1,0) circle (1.5pt) node[left] {\small $(l,0)$};

% Parallelogram region
\fill[orange!20, opacity=0.6] (-1,0) -- (-1,0.8) -- (2,2) -- (2,0) -- cycle;

% Linear bounds
\draw[orange!80!black, thick, dashed] (-1,0) -- (2,2); % upper bound
\draw[orange!80!black, thick, dashed] (-1,0.8) -- (2,2); % lower bound

% Labels
\node[orange!60!black] at (1.2,1.8) {\small upper bound};
\node[orange!60!black] at (0.3,0.6) {\small lower bound};
\node at (1.6,-0.3) {\small unstable zone};

\end{tikzpicture}
\caption{ReLU function (black) and its zonotope over-approximation (orange parallelogram) for an input interval crossing zero.}
\label{fig:relu-parallelogram}
\end{figure}

\paragraph{Geometric picture.}
When \([l,u]\) crosses zero, we replace the “kink’’ of ReLU with a tilted parallelogram bounded by two lines:
- one that under-approximates (the lower slope \(\lambda x\)),
- one that over-approximates (the upper line \(\lambda x + u(1-\lambda)\)).

This parallelogram fully contains the real ReLU curve, and by converting it into a zonotope, we get a linear representation that can still be propagated.

\paragraph{Key insight.}
Each ReLU that crosses zero introduces one new generator — representing the uncertainty of whether that neuron is active or inactive.  
As a network gets deeper, the number of generators grows, each tracking different combinations of “maybe-active’’ neurons.  
This is what allows zonotope-based verifiers to remain precise even after many nonlinear layers.

\begin{example}[ReLU on zonotope]\label{ex:zonotope_relu_example}
...
\end{example}

\paragraph{Summary intuition.}
Intervals treat each neuron’s uncertainty separately → boxes.  
Zonotopes tie them together → tilted boxes (parallelograms).  
Through affine layers, zonotopes move exactly; through ReLUs, they get conservatively widened by adding one extra generator per uncertain neuron.  
Over multiple layers, this tradeoff between precision and complexity makes zonotopes one of the most widely used abstractions in neural network verification.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{ReLU Transformer}

Activation functions like ReLU are non-affine and do not preserve zonotope structure\tvn{what is it? can we draw it}. For the ReLU function \(\text{ReLU}(x) = \max(0, x)\), we must use conservative approximations. Given an input zonotope\tvn{why not just interval?}, we first compute its interval bounds \([l, u]\) (\tvn{like what we just computed in 5.2.2.1?}), then consider three cases:


\begin{itemize}
    \item \textbf{Active case} (\(l \geq 0\)): All values are non-negative, so \(\text{ReLU}^a(x) = x\) (identity).
    \item \textbf{Inactive case} (\(u \leq 0\)): All values are negative, so \(\text{ReLU}^a(x) = \{0\}\) (zero zonotope).
