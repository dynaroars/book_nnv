...
Note that because $l_3 = 0$, $x_3$ can never be negative, so the active phase ($a_3 = 1$) is always feasible, and the inactive phase ($a_3 = 0$) is infeasible. The MILP encoding of $x_3$ can then be simplified to:
\begin{align*}
    &\hat{x}_3 \geq x_3 \\
    &\hat{x}_3 \geq 0 \\
    &\hat{x}_3 \leq 1.0 \\
    ...
    &a_3 = 1
\end{align*}
This means that $\hat{x}_3 = x_3$ and $0.0 \le x_3 \le 1.0$ for any inputs $x_1 \in [-1,1]$ and $x_2\in[-2,2]$.
\end{example}    
