...
These constraints enforce $\hat{z} = z$ when $a = 1$ (active, $z \geq 0$) and $\hat{z} = 0$ when $a = 0$ (inactive, $z < 0$), which capture the semantics of ReLU.
...
To determine the pairs of values that achieve the lower and upper bounds of a linear expression such as
\[
x_3 = a_1 x_1 + a_2 x_2 + \ldots + b,
\]
where each variable $x_i$ ranges over $[l_i, u_i]$, proceed as follows:
...
This guarantees that the extreme values of $x_3$ are achieved at some corner of the input box, with the exact pair determined by the sign of each coefficient.
...
Note that because $l_3 = 0$ and $u_3 = 1$, $a_3$ can be either 0 or 1, depending on the value of $x_3$. This is actually a worst-case scenario for the MILP solver, as it has to consider both cases. If we had different bounds, e.g., $l_3 = 0.1$ and $u_3 = 0.9$, then $a_3$ would be forced to be 1, as $x_3$ can never be less than 0.1, and the MILP solver would only have to consider the active case.
\end{example}
...
%\tvn{Hai: add limitations to MILP solving, e.g., why can't we use this method of encoding for verifying DNNs? why not just let the MILP solver decides which branch to take (e.g., active or inactive)?  Why people need to create their own Brand and Bound or Reluplex algorithms?  Why do we need new abstraction and this interval abstraction is not sufficient for computing bounds? Provide simple examples would help demonstrate what you mean.}
%\hd{simple example will not be able to show the limitation, e.g., interval vs polytope are likely the same on small networks}



\begin{itemize}
    \item \textbf{Scalability:} While MILP is more efficient than SMT solving, it still cannot be applied directly to real-world DNNs due to the exponential growth of the search space. Each ReLU introduces a binary variable, leading to $2^n$ possible activation patterns for $n$ ReLUs.

    \item \textbf{Limited exploitation of network structure:} Industrial MILP solvers, such as Gurobi~\cite{gurobi}, are highly optimized for general problems but do not exploit the layer-wise structure, sparsity, or other properties unique to neural networks.

    \item \textbf{Inefficient parallelism:} MILP solvers are primarily CPU-based and does not leverage the massive parallelism provided by modern GPUs.
    ...
    \item \textbf{Advanced Abstraction:} Interval analysis is commonly used for computing neuron bounds, but it suffers from precision loss that accumulates across layers. Intervals cannot capture dependencies between neurons, leading to loose over-approximations in deeper networks. To address the limitations of intervals, more advanced abstract domains such as zonotopes (which capture linear relationships) and polytopes (which represent arbitrary linear constraints) are employed in state-of-the-art verification methods.
\end{itemize}
