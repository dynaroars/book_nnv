...
\]
\tvn{what are these variables and what does this property say at high level?}
...
We now define abstract transformers for the interval domains for affine and ReLU activation functions.
...
Consider the DNN in~\autoref{fig:dnn-abs} with inputs \(x_1 \in [1, 2]\) and \(x_2 \in [-1, 3]\). We can compute the lower and upper bounds of the output of neuron \(x_3\) using the abstract transformer:
