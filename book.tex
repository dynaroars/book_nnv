...
\subsection{Conjunctive Normal Form (CNF)} A formula is in CNF if it is a \emph{conjunction} of  \emph{clauses}, where each clause is a \emph{disjunction} of \emph{literals}, where a literal is either a variable $p$ or its negation $\lnot{p}$. 
...
This is equivalent to saying that its negation $\lnot{\alpha}$ is unsatisfiable (no assignment satisfies it), i.e., 
\[
\text{valid}(\alpha) \iff \text{unsat}(\lnot{\alpha})
\]  
Thus, to check the validity of $\alpha$, we can use a SAT solver to check that $\lnot{\alpha}$ is \unsat{}.
