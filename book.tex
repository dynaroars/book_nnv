...
At the heart of every FM is a \emph{specification}---a precise description of what it means for a system to be correct (or safe, or robust, etc.). A specification defines the properties we want to verify about a system.

Software engineers often write specifications informally in natural language. However, these informal specifications are often ambiguous and imprecise, making them vulnerable to misinterpretation and error. 
For example, we might say: the system should be ``fast'' or ``robust.'' But what does that mean exactly? How fast is fast enough? What kinds of perturbations count as robust?  Or for a simple sorting algorithm, we might say: it should ``sort the list correctly.'' But what does ``correctly'' mean? Should it be sorted in ascending or descending order? Should it handle duplicate values?
...
For example, an informal specification would say``Small changes to the input image shouldn't cause the neural network to change its classification result'', while a formal specification of robustness would instead say:

% > For a specific input image x, for all images x′ such that the distance between x and x′ is at most ε, the network’s output label for x′ must be the same as for x.

% This statement is precise. It defines exactly what inputs are allowed and exactly what behavior is required. Because of this precision, it can be proved or disproved.

% Without a specification like this, verification cannot even begin. Formal methods do not answer the question “Is this system good?” They answer the question “Does this system satisfy this specific property?”

% ---

% ## 3. Testing, Debugging, and Verification: A Fundamental Difference

% Many students believe verification is simply testing taken to an extreme. This is incorrect. Testing, debugging, and verification answer different questions.

% ### Testing

% Testing evaluates a system by running it on a **finite set of concrete inputs**. In software engineering, this includes unit tests and integration tests. In machine learning, this includes validation and test datasets.

% For example, suppose we train a neural network to recognize handwritten digits. We might test it on 10,000 labeled images and observe that it achieves 99% accuracy. This result tells us that the model performs well on those images. It does not tell us how the model behaves on inputs we did not test.

% The key limitation of testing is that it can only sample the input space. For neural networks, the input space is usually enormous or continuous, making exhaustive testing impossible.

% ### Debugging

% Debugging is what we do after a test fails. We inspect the system, identify the cause of the failure, modify the system, and test again. Debugging improves systems, but it does not provide guarantees. It fixes known failures, not unknown ones.

% ### Verification

% Verification asks a fundamentally different question:

% > Does the system satisfy the specification for **all possible inputs**?

% Instead of running the system on selected inputs, verification reasons symbolically about its behavior. If verification succeeds, the result is a guarantee: the property holds for every input allowed by the specification.

% Consider a neural network controlling the speed of a drone. Testing might involve simulating many flight scenarios. Verification might instead prove the statement:

% > For all sensor readings within specified ranges, the commanded speed never exceeds a safe maximum.

% This guarantee applies even to scenarios that were never simulated or imagined.

% This is why formal verification is used in safety-critical systems and why neural network verification has become important as learned models are deployed in such settings.
