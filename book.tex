...
\begin{example}\label{ex:relu_milp}
...
Use MILP encoding and solving to check if the network in~\autoref{fig:dnn-d} satisfy the property $\phi$ that the output $x_5 \ge x_6$ for any inputs $x_1 \in [-1,1], x_2\in[-1,1]$. Specificaly, do the following steps:
\begin{enumerate}
...
\begin{itemize}
    \item \textbf{Scalability:} While MILP focuses on linear constraints and is generally more efficient than general SMT solving, it still cannot be applied directly to real-world DNNs due to the exponential growth of the search space. Each ReLU introduces a binary variable (\autoref{sec:relu-encoding}), leading to $2^n$ possible branches, and real-world DNNs can have millions of ReLUs, making the search space intractable.

    \item \textbf{Limited exploitation of network structure and modern hardware} Off-the-shelf MILP solvers are designed for arbitrary MILP problems and do not exploit DNN-specific structures such as\tvn{Hai, fill int}\hd{FNNs, CNNs, ResNets, etc.}
...
As mentioned in~\autoref{sec:relu-encoding} we need to compute the bounds of the neurons to reduce the search space. Modern DNN verifiers use \emph{abstraction} techniques to compute these bounds more precisely while balancing computational efficiency.
This chapter discusses several \emph{abstraction domains} used in DNN verification, including interval, zonotope, and polytope abstractions.
...
Then the bounds for \(x_3\) can be computed as:
\[
\begin{aligned}
...
\begin{example}
For~\autoref{ex:transformer-affine1}, applying ReLU$^a$ to neuron $x^3$ gives:
...
Intervals is very \emph{efficient} and scales  well to large networks. The affine transformer only requires a linear number of multiplications and min/max operations, and ReLU reduces to only three matching cases.
...
Moreover, if we apply ReLU, then the output would be $\myrelu(z) = \myrelu(0) = 0$, but the interval abstraction gives $\myrelu^a([-1, 1]) = [0, 1]$, which is again a loose over-approximation.
\end{example}

Interval overapproximation grows quickly as we propagate through many layers of a large network, i.e., it keeps ``inflating'' the bounds, leading to a very loose approximation of the output and becomes useless for verification tasks. For example, $z \leq 0$ is valid for~\autoref{ex:interval-overapproximation}, but the interval abstractions gives $z \in [0,1]$, which is not tight (precise) enough to verify this property.

Nonetheless, interval-based methods remain popular due to their simplicity and efficiency. In some cases, despite being too coarse, they can still successfully verify properties of neural networks, e.g., if we want to verify that $z \le 2$, then the interval $z\in [0, 1]$ would suffice.
...
An interval abstraction (\autoref{sec:interval-abstraction}) treats each variable independently. For example, if \(x_1 \in [1,2]\) and \(x_2 \in [3,4]\), intervals assume any combination of \(x_1\) and \(x_2\) is possible. But in many real systems, variables correlate: when \(x_1\) increases, \(x_2\) also increase.  Thus, zonotopes, which can capture such correlations, provide a tighter abstraction.
...
In geometric terms, as shown in \autoref{fig:zonotope-vs-box}, while intervals form axis-aligned boxes, zonotopes form \emph{tilted boxes} (parallelograms or higher-dimensional analogues) that can stretch, rotate, and capture relationships between variables (like ``when $x_1$ goes up, $x_2$ also goes up'').
More specifically, zonotopes extend intervals by introducing \emph{generators}, which are vectors that define directions of variability and allow the zonotope to tilt and stretch in directions that capture correlations between variables. 




\paragraph{Definition}
    A zonotope is built by taking a ``center point'' and adding several small ``generator'' directions that can each vary a bit in both directions (positive and negative). In addition, each variation is controlled by a coefficient \(\epsilon_i\) serving as a ``knob'' to adjust how much we move along each generator direction.
    More specifically, a \textbf{zonotope} \(\mathcal{Z}\) in \(\mathbb{R}^n\) is defined as:
\[
...
\item \(\epsilon_i\) are independent coefficients in \([-1, 1]\) that determine how much we move along each generator direction. E.g., -1 means moving in the negative direction of the generator, 0 means no movement, and +1 means moving in the positive direction. 
\end{itemize}

% \paragraph{A simpler geometric example.}
% In 2D, let \(c = (0,0)\), \(g_1 = (1,0)\), and \(g_2 = (1,1)\).  
% Then
% \[
% \mathcal{Z} = \{ (0,0) + \epsilon_1(1,0) + \epsilon_2(1,1) \mid \epsilon_i \in [-1,1]\}
% \]
% This produces a parallelogram, not a square — since both \(x\) and \(y\) depend on \(\epsilon_2\), movement in \(x\) is correlated with movement in \(y\).  
% That “tilt’’ captures dependency that intervals cannot.




\begin{example}[Interval is a special case of zonotope]




     An interval \([l, u]\) can be represented as a zonotope with center \(c = \frac{l+u}{2}\) and a single generator \(g = \frac{u-l}{2}\), i.e., 

     \[\mathcal{Z} = \left\{ c + \epsilon g \mid \epsilon \in [-1, 1] \right\} = \left\{ \frac{l+u}{2} + \epsilon \frac{u-l}{2} \mid \epsilon \in [-1, 1] \right\} = [l, u]\]    

     Thus, zonotopes generalize intervals by allowing multiple generators to capture more complex dependencies.
\end{example}
...
x \in [-2, 3].
\]
We can express this interval as a zonotope with one generator as shown in \autoref{fig:interval-zonotope}. 
...
Affine operations preserve the structure of zonotopes exactly: no approximation needed.  
...
Consider the DNN in~\autoref{fig:dnn-abs} with inputs \(x_1 \in [1, 2]\) and \(x_2 \in [-1, 3]\). The affine function for neuron \(x_3\) is:
\[
f(x_1,x_2) = -0.5\,x_1 + 0.5\,x_2 + 1.0
\]
...
\]
For the weight \(w = [-0.5, 0.5]^\top\) and bias \(b = 1.0\), applying the affine transformer:
\begin{equation}
\begin{aligned}
...
\subsubsection{ReLU Transformer}
<<<<<<< HEAD
\label{sec:zonotope-relu-transformer}
Activation functions like ReLU are non-affine and do not preserve zonotope structure.  
For the ReLU function \(\text{ReLU}(x) = \max(0, x)\), the shape of the input region changes at \(x = 0\).  
When a zonotope crosses this point, its smooth tilted-box form becomes bent:  
part of it is flattened (where \(x < 0\)), while the rest stays slanted (where \(x > 0\)).  
This bending makes the ReLU output no longer a zonotope, so we need an overapproximation that captures all possible outputs.
...
%   % axes
%   \draw[->, thick] (-1.5,0) -- (2.6,0) node[right] {$x$};
%   \draw[->, thick] (0,-0.5) -- (0,2.5) node[above] {$y=\mathrm{ReLU}(x)$};

%   % ReLU curve
%   \draw[thick, black] (-1.2,0) -- (0,0);
%   \draw[thick, black] (0,0) -- (2.4,2.4);

%   % input zonotope band
%   \fill[blue!10, opacity=0.6] (-1.0,-0.4) -- (2.0,1.6) -- (2.0,2.0) -- (-1.0,0.0) -- cycle;
%   \draw[blue!60!black, thick] (-1.0,-0.4) -- (2.0,1.6);
%   \draw[blue!60!black, thick] (-1.0,0.0) -- (2.0,2.0);

%   % output bent region
%   \fill[orange!20, opacity=0.6] (-1.0,0.0) -- (0,0.0) -- (2.0,2.0) -- (2.0,1.6) -- (0,0.0) -- cycle;

%   % labels
%   \node[blue!60!black] at (-0.7,1.2) {\small input};
%   \node[orange!80!black] at (1.4,1.7) {\small after ReLU (bent)};
%   \fill (0,0) circle (1.5pt) node[below right] {\small $(0,0)$};
% \end{tikzpicture}
% \caption{When a zonotope crosses zero, the ReLU “bends” its shape: the negative part collapses to $0$, while the positive part remains slanted.}
% \label{fig:relu-bend}
% \end{figure}
...
We can distinguish three possible cases of ReLU depending on the interval bounds $[l,r]$ of the input zonotope.
...
\fill[orange!20, opacity=0.6] (-1,0) -- (2,2) -- (2,1.3) -- (-1,-0.7) -- cycle;

% Linear bounds
\draw[orange!80!black, thick, dashed] (-1,0) -- (2,2); % upper bound
\draw[orange!80!black, thick, dashed] (-1,-0.68) -- (2,1.32); % lower bound

% Labels
\node[orange!60!black] at (0.7,1.8) {\small upper bound};
\node[orange!60!black] at (2.0,0.6) {\small lower bound};
% \node at (1.6,-0.3) {\small unstable zone};
...
\fill[orange!20, opacity=0.6] (-1.0,0.0) -- (-1.0,0.8) -- (2.0,2.0) -- (2.0,0.0) -- cycle;
\draw[orange!70!black, dashed, thick] (-1.0,0.0) -- (2.0,2.0);
\draw[orange!70!black, dashed, thick] (-1.0,0.8) -- (2.0,2.0);

% labels
\node[orange!80!black] at (1.0,1.3) {\small parallelogram approximation};
\node[black] at (-1.2,-0.3) {\small $l$};
\node[black] at (2.2,-0.3) {\small $u$};
\fill (0,0) circle (1.5pt);
\node[below right] at (0,0) {\small $(0,0)$};
=======
\draw[very thick, black, domain=-2.0:0] plot (\x,0);
\draw[very thick, black, domain=0:2.5] plot (\x,\x);

% Points
\fill (0,0) circle (1.5pt) node[below left] {\small $0$};
\fill (2,2) circle (1.5pt) node[right] {\small $(u,u)$};
\fill (-1,0) circle (1.5pt) node[below] {\small $(l,0)$};
>>>>>>> 0b0487c019295f0aa302aa8adbbfad4a3f4a93d5

\end{tikzpicture}
\caption{Two linear constraints (orange) form an upper and lower bound enclosing the true ReLU (black) over $[l,u]$. The shaded parallelogram is the region we use for zonotope over-approximation.}
\label{fig:relu-parallelogram}
\end{figure}

For the unstable case, we need to over-approximate the ReLU function \(y = \max(0, x)\) over the interval \([l, u]\) where \(l < 0 < u\).  
Since the ReLU function is non-convex (it has a corner at \(x = 0\)), we cannot represent it exactly with a zonotope.  
Instead, we construct a \emph{parallelogram} that contains all possible ReLU outputs.

The zonotope approximation uses two linear constraints to bound the ReLU:
\begin{itemize}
    \item \textbf{Lower bound constraint:} \(y \geq \lambda x\), where \(\lambda = \frac{u}{u-l}\) is the slope of the line connecting points \((l, 0)\) and \((u, u)\).
    \item \textbf{Upper bound constraint:} \(y \leq \lambda x + u(1-\lambda)\), which is the line with slope \(\lambda\) passing through \((u, u)\).
\end{itemize}

These two lines form the upper and lower edges of the parallelogram.  
Geometrically, the lower line starts from the left corner of the ReLU (where \(x=l\)) and tilts upward with slope \(\lambda\).  
The upper line touches the ReLU at the right endpoint \((u, u)\).  
The region between them covers all possible outputs.


This creates a parallelogram that:
\begin{enumerate}
    \item Contains the actual ReLU function over \([l, u]\)
    \item Has vertices at \((l, 0)\), \((l, u(1-\lambda) - \lambda l)\), \((u, u)\), and \((u, 0)\)
    \item Introduces approximation error in the small triangular gap between the linear bounds and the true curve
\end{enumerate}


% \textbf{Intuition:}  
% The parallelogram acts like a “tilted wrapper” around the ReLU’s bent region.  
% Its slope \(\lambda\) makes the shape narrower when the interval \([l,u]\) is large (more uncertainty) and steeper when the input is mostly positive (less uncertainty).  
% By ensuring the real ReLU curve is always inside this parallelogram, we guarantee the output is safely over-approximated.



To represent this parallelogram as a zonotope, we transform the input zonotope \((c, G)\) as follows:
\begin{itemize}
    \item \textbf{Scale existing generators:} Each generator \(g_i\) becomes \(\lambda g_i\)
    \item \textbf{Transform center:} \(c' = \lambda c + \frac{u(1-\lambda)}{2}\)
    \item \textbf{Add new generator:} \(g_{new} = \frac{u(1-\lambda)}{2}\) to capture the approximation error
\end{itemize}

The final zonotope after ReLU transformation becomes \((c', G')\) where:

\textbf{Final center:}
\[
c' = \lambda c + \frac{u(1-\lambda)}{2}
\]

\textbf{Final generator matrix:}
\[
G' = [\lambda g_1, \lambda g_2, \ldots, \lambda g_m, g_{new}]
\]

where the original generators \(g_1, g_2, \ldots, g_m\) are scaled by \(\lambda\), and a new generator \(g_{new} = \frac{u(1-\lambda)}{2}\) is appended.

\vspace{0.5em}
\noindent
\textbf{Intuition:}  
- The scaling by \(\lambda\) compresses the zonotope in the $x$ direction, matching the slope of the ReLU’s linear bounds.  
- The offset $\frac{u(1-\lambda)}{2}$ shifts the center upward, so it lies in the middle of the parallelogram.  
- The new generator \(g_{new}\) adds a small “vertical wiggle” that represents the uncertainty caused by the triangular error area we replaced with a rectangle.

\begin{figure}[h!]
\centering
\begin{tikzpicture}[scale=1.1,>=stealth]
  % Axes
  \draw[->, thick] (-1.5,0) -- (2.8,0) node[right] {$x$};
  \draw[->, thick] (0,-0.5) -- (0,2.5) node[above] {$y$};

  % Parallelogram (final zonotope)
  \fill[orange!20] (-1.0,0.0) -- (-1.0,0.8) -- (2.0,2.0) -- (2.0,0.0) -- cycle;
  \draw[orange!80!black, thick] (-1.0,0.0) -- (2.0,2.0);
  \draw[orange!80!black, thick] (-1.0,0.8) -- (2.0,2.0);

  % center line
  \draw[dashed, blue!60!black] (-1.0,0.4) -- (2.0,1.8);
  \filldraw[blue!60!black] (0.5,1.1) circle (1.5pt);
  \node[blue!60!black] at (0.6,1.4) {\small new center $c'$};
  \node[orange!80!black] at (1.5,0.4) {\small new generator $g_{new}$};
\end{tikzpicture}
\caption{After approximation, the bent ReLU region is represented by a new zonotope (orange parallelogram). The center $c'$ is shifted, old generators are scaled, and one new generator $g_{new}$ is added to capture the approximation error.}
\label{fig:relu-zonotope-final}
\end{figure}

% \vspace{0.5em}
% \noindent
% \textbf{Takeaway:}  
% Zonotopes handle affine transformations exactly but approximate nonlinear ones like ReLU by safely wrapping them in linear envelopes.  
% Each time a neuron crosses zero (the unstable case), a new generator is added to capture that small uncertainty.  
% Over multiple layers, these generators accumulate—but the result is always a convex, compact shape that soundly over-approximates the DNN’s behavior.


\subsubsection{ReLU Transformer}
<<<<<<< HEAD
\label{sec:zonotope-relu-transformer}
Unlike affine transformations, nonlinear activations like ReLU do not preserve the zonotope structure.
For an input zonotope with lower and upper bounds \([l, u]\), we consider three cases of ReLU$^a$:
=======

Activation functions like ReLU are non-affine and do not preserve zonotope structure\tvn{what is it? can we draw it}. For the ReLU function \(\text{ReLU}(x) = \max(0, x)\), we must use conservative approximations. Given an input zonotope\tvn{why not just interval?}\hd{no intervals here. Only propagate zonotopes, intervals only are computed to decide which neurons are stable or unstable}, we first compute its interval bounds \([l, u]\) (\tvn{like what we just computed in 5.2.2.1?})\hd{no, zonotope uses centers and generators, so computing like~\autoref{ex:zonotope_linear_example}}, then consider three cases:
>>>>>>> 0b0487c019295f0aa302aa8adbbfad4a3f4a93d5


\begin{itemize}
    \item \textbf{Active case} (\(l \geq 0\)): All values are non-negative, so \(\text{ReLU}^a(x) = x\) (identity function), i.e., the output zonotope is the same as the input zonotope.
    \item \textbf{Inactive case} (\(u \leq 0\)): All values are negative, so \(\text{ReLU}^a(x) = \{0\}\), i.e., the output zonotope is a single point at zero.
    \item \textbf{Unstable case} (\(l < 0 < u\)): The range crosses zero, requiring over-approximation using a parallelogram that bounds ReLU over \([l, u]\) as described below.
\end{itemize}

For the unstable case, we need to over-approximate the ReLU function \(y = \max(0, x)\) over the interval \([l, u]\) where \(l < 0 < u\). Since the ReLU function is non-convex (it has a corner at \(x = 0\)), we cannot represent it exactly with a zonotope. Instead, we construct a parallelogram that contains all possible ReLU outputs.
...
\begin{enumerate}
    \item Contains the actual ReLU function over \([l, u]\)
    \item Has vertices at \((l, 0)\), \((l, u(1-\lambda) - \lambda l)\), \((u, u)\), and \((u, 0)\)
    \item Introduces approximation error in the triangular region between the constraints
\end{enumerate}
...
The new generator is essential because the ReLU over-approximation creates a triangular error region between the actual ReLU function and our linear bounds. However, zonotopes can only represent parallelograms formed by linear combinations of generators. To capture this triangular error within the zonotope framework, we must ``rectangularize'' the triangle by introducing a new generator that spans the height of the approximation error.


\begin{example}[ReLU on zonotope]\label{ex:zonotope_relu_example}
Continuing from the previous example, applying ReLU to the zonotope output \((0.75, [-0.25, 1.0])\). 

First, we compute the concrete bounds from this zonotope representation:
\begin{align}
l &= c - |g_1| - |g_2| = 0.75 - |-0.25| - |1.0| = 0.75 - 0.25 - 1.0 = -0.5 \\
u &= c + |g_1| + |g_2| = 0.75 + |-0.25| + |1.0| = 0.75 + 0.25 + 1.0 = 2.0
\end{align}

So the input to ReLU has bounds \([-0.5, 2.0]\). Since \(l = -0.5 < 0 < 2.0 = u\), this is an unstable neuron requiring over-approximation.

\textbf{Step 1: Slope approximation}
We use slope \(\lambda = \frac{u}{u-l} = \frac{2.0}{2.0-(-0.5)} = \frac{2.0}{2.5} = 0.8\).

\textbf{Step 2: Transform zonotope}
The new center becomes:
\[
c' = \lambda c + \frac{u(1-\lambda)}{2} = 0.8 \cdot 0.75 + \frac{2.0 \cdot 0.2}{2} = 0.6 + 0.2 = 0.8
\]
...
\textbf{Step 3: New generator for approximation error}
A new generator is introduced to capture the ReLU approximation error:
\[
g'_3 = \frac{u(1-\lambda)}{2} = \frac{2.0 \cdot 0.2}{2} = 0.2
\]

\textbf{Result:} The output zonotope is \((0.8, [-0.2, 0.8, 0.2])\) with concrete bounds:
\begin{align*}
...
\part{DNN Verification Algorithms}\label{part:algorithms}
\chapter{The Branch and Bound Search Algorithm}\label{chap:bab}

As shown in~\autoref{chap:constraint-solving}, the NNV verification problem can be formulated as a satisfiability problem, solvable using a constraint solver.  However, constraint solvers, e.g., SMT and MILP solvers, do not scale to large DNNs due to the complexity of the underlying formulae. Thus, modern DNN verification techniques reframe the problem to search for \emph{activation patterns} that satisfy the constraints, and use the \emph{Branch-and-Bound} algorithm to explore the space of possible activation patterns.
...
Thus, verifying a DNN reduces to checking that none of these linear regions contains a counterexample. More specifically, we can rewrite~\autoref{eq:nnv2} as:
...
Consider the DNN in \autoref{fig:dnn-b}. Suppose we fix the activation pattern
\[
...
\section{Branch and Bound}\label{sec:bab-alg}

Modern DNN verifiers typically adopt the Branch-and-Bound (BaB) approach to explore the space of possible neuron activation patterns. The BaB algorithm systematically splits (\emph{branch}) the verification problem into smaller subproblems by deciding the activation status (active/inactive) of neurons, and uses abstraction (bound) techniques to compute upper and lower bounds on the output values for these subproblems. If a subproblem's bounds indicate that it cannot contain a counterexample, it is pruned from the search space. This process continues until either a counterexample is found or all subproblems are exhausted, proving the property holds.
...
In each iteration, \bab{} selects and removes an activation pattern $\sigma_i$ from \texttt{ActPatterns}.  It then calls \functiontextformat{Deduce} to check the feasibility of the problem based on the current activation pattern. (Note that we do this even on the empty activation pattern, which is the initial state of the search, because the problem might be trivially infeasible.)
...
\begin{figure}
    \centering
    \proofnet{1}
    \caption{A simple DNN (similar to~\autoref{fig:proofnet})}\label{fig:example1}
\end{figure}
...
\begin{itemize}
    ...
    \item Programming Assignment 1 (PA1): Symbolic Execution of Neural Networks~\autoref{sec:pa1}. Due date \alert{Monday October 6, 2025 11:59pm}.
    ...
    \end{itemize}
    \item Programming Assignment 2 (PA2): Abstraction~\autoref{sec:pa2}. Due date \alert{Wednesday October 29, 2025 11:59pm}
    \item Week 8 (Oct 15):
    \begin{itemize}
...
\section*{Grading Rubric \color{red}{(not yet updated for PA2)}}
...
% Encode in MILP:  
% “If factory A is open ($z_A=1$) then production $\geq 50$,  
% if factory B is open ($z_B=1$) then production $\geq 30$.  
% At most one factory may be open.”
...
u  = x3 =  -0.5(-1) + 0.5(2) + 1  =  2.5 
l = x3 = -0.5(1) + 0.5(-2) + 1  =  -0.5