...
Interval overapproximation grows quickly as we propagate through many layers of a large network, i.e., it keeps ``inflating'' the bounds, leading to a loose approximation of the output. For example, $z \leq 0$ is a valid problem for~\autoref{ex:interval-overapproximation}, but the interval abstractions gives $z \in [0,1]$, which is not tight (precise) enough to show this property. 
...
An interval abstraction (\autoref{sec:interval-abstraction}) treats each variable independently. For example, if \(x_1 \in [1,2]\) and \(x_2 \in [3,4]\), intervals assume any combination of \(x_1\) and \(x_2\) is possible. But variables can correlate: when \(x_1\) increases, \(x_2\) also increase.  Thus, zonotopes, which can capture such correlations, can provide a tighter abstraction.
...
\paragraph{The generator-based representation} There are two common ways to describe a geometric shape (like rectangle, parallelograms, or higher-dimensional polyhedra):
\begin{itemize}
  \item \textbf{Vertex-based representation:} Define the shape by listing all of its corner points (vertices). For example, a rectangle in 2D is defined by its four corners, and a box in 3D is defined by its eight corners. This representation is intuitive and directly shows the boundaries of the shape.
        
  \item \textbf{Generator-based representation:} Define the shape using a \emph{center point} and several direction vectors (\emph{generators}) that define how the shape can stretch or tilt. Instead of listing all corners, it specifies how to reach any point in the shape by starting from the center and moving along the generators within certain limits ($\epsilon$).  This representation is often more compact, especially for higher-dimensional shapes.
\end{itemize}

\autoref{fig:interval-rectangle-two-views} uses vertex-based and generator-based representations to represent a 2D rectangle (an interval abstraction for two variables). 
...
\begin{example}
  An interval for a single variable is simply a line segment defined by its \emph{two} endpoints (corners)---the lower and upper bounds. Equivalently, it can be represented using a center point and a single generator vector that points in both directions (left and right) from the center:
  \[
...
\paragraph{Definition of Zonotope}

A zonotope is a geometric object that generalizes the idea of an interval to multiple dimensions.
A zonotope is built by starting from a center point and then stretching along several directions (generators).
...
\begin{example}[2D Zonotope]
  A parallelogra is zonotope in 2D as shown in~\autoref{fig:zonotope-two-views}. It can be defined by its center point and two generator vectors that define its sides. For instance, if the center is at \(c = (1, 1)\) and the generators are \(g_1 = (2, 0)\) and \(g_2 = (0, 1.5)\), then the zonotope represents all points that can be reached by moving along these generators within the range \([-1, 1]\):
  \[
    \mathcal{Z} = \left\{ (1, 1) + \epsilon_1 (2, 0) + \epsilon_2 (0, 1.5) \mid \epsilon_1, \epsilon_2 \in [-1, 1] \right\}
  \]
  This zonotope includes points like:
    \[
      (1, 1) + (-1)(2, 0) + (1)(0, 1.5) = (-1, 2.5)
    \]
    \[
      (1, 1) + (1)(2, 0) + (-1
      )(0, 1.5) = (3, -0.5)
...
\begin{scope}[shift={(3.8,0)}]
  ...
  \draw[->,blue!70!black,thick] (c) -- ++(0.4,1.0) node[above right] {\small $g_2$};

  % Labels
  \node[orange!80!black] at (1.4,1.8) {\small Generator-based view};
  \node at (1.2,-0.4) {\small Built from $c+\epsilon_1g_1+\epsilon_2g_2$};
\end{scope}

% Connecting text
\node at (0,0.8) {\Large $\Longleftrightarrow$};
