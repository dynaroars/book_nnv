...
A fully connected feed-forward neural network (FNN), shown below, is an FFN where every neuron in a layer is connected to every neuron in the next layer. \autoref{fig:dnn} is a simple FNN.
\begin{center}
...
\end{itemize} 
The ONNX representation would look like:\tvn{could we put in the exact ONNX code?}
...
In SMT-LIB\tvn{is this also the VNN-LIB code? If not, add the VNN-LIB code}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
(declare-fun X0 () Real)
...
...
\tvn{Need the VNN-LIB code here.}
\end{example}

\subsubsection*{Variable Naming Convention}\tvn{what's the purpose of this section?}
\begin{itemize}
