...
\paragraph{ReLU Encoding} More specifically, to encode ReLU, we first introduce the following notation:
\begin{itemize}
...
The MILP encoding of $\hat{z} = \max(z, 0)$ is:
\begin{align*}
    &\hat{z} \geq z \\
    &\hat{z} \geq 0 \\
    &\hat{z} \leq a \cdot u \\
...
\begin{itemize}
    \item If $a = 1$ (active, $z \geq 0$): the constraints enforce $\hat{z} = z$
    \item If $a = 0$ (inactive, $z < 0$): the constraints enforce $\hat{z} = 0$
\end{itemize}
This encoding allows the MILP solver to reason precisely about the nonlinearity using only linear constraints and binary (integer) variables.




The general MILP encoding of a DNN is as follows:
...
\begin{example}\label{ex:milp-dnn}
    For the example in~\autoref{ex:smt-dnn}, we have the following MILP formulation representing the DNN in \autoref{fig:dnn-a} and the property $x_5 > 0$ for any inputs $x_1 \in [-1,1], x_2\in[-2,2]$:
