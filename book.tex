...
\]
\begin{enumerate}
  \item Provide the constraints induced by $p$.
...
It then calls \mycode{Deduce} to \emph{quickly} determine if the current problem, i.e., the original satisfiability problem with  activation pattern $\sigma_i$,
...
\begin{itemize}
\item If \mycode{Deduce} determines that the problem is infeasible, \bab{} discards the current processing activation pattern and loops back (to~\autoref{line:babstart}) to process the next activation pattern. In other words, it prunes the current search branch with activation pattern $\sigma_i$.
...
\item If \mycode{Deduce} determines that the problem is feasible, \bab{} checks the satisfiability of the problem using an LP solver (\autoref{line:bablp})\footnote{The problem is formulated as a MILP problem as described in~\autoref{sec:milp-formulation} and checked using an LP solver. Note that we use LP and MILP interchangeably here for simplicity. In practice, most LP solvers such as Gurobi handles MILP problems as well. Just like SMT solvers handle both SAT and SMT problems.}. If the solver finds a satisfying assignment, it returns \sat{} and the counterexample represented by the satisfying assignment (\autoref{line:babsat}), indicating that the property is invalid.
Otherwise, \mycode{LP} returns unsatisfiable, and \bab{} calls \mycode{Decide} to select a neuron $v_i$ to split, i.e., to fix the activation status of $v_i$ as either active or inactive (\autoref{line:babdecide}).  This means the problem is split into two independent subproblems: one with $v_i$ (active) and the other with $\overline{v_i}$ (inactive). \bab{} then adds the two new activation patterns $\sigma_i \land v_i$ and $\sigma_i \land \overline{v_i}$ to \texttt{ActPatterns}. \bab{} then loops back to~\autoref{line:babstart} to process the next activation pattern.
\end{itemize}


\paragraph{\textbf{Deduce vs. LP}} Note the difference between \mycode{Deduce} and \mycode{LP} is that the former uses abstraction to compute over-approximated bounds to determine feasibility, while the latter uses an LP solver to check exact satisfiability. 
...
First, \bab{} initializes \mycode{ActPatterns} with an empty activation pattern $\emptyset$ (i.e., no neurons fixed). Then \bab{} enters its main loop. In the first iteration, \bab{} selects the only available activation pattern $\emptyset$, i.e., $\sigma_i = \emptyset$,
and  calls \mycode{Deduce} to check the feasibility of the problem based $\sigma_i$.

Here, \mycode{Deduce} determines that the problem is feasible, e.g., the bounds of $y_1$ and $y_2$ indicate that $y_1 \le y_2$ can be satisfied. Thus, \bab{} calls \mycode{LP} to check the satisfiability of the problem using an LP solver. The LP solver returns unsatisfiable (or unknown)--meaning no counterexample is found in this activation pattern.  \bab{} then randomly selects a neuron to split (\mycode{Decide}). Assume that it chooses $v_4$ to split, i.e., the problem spawns two independent subproblems: one with $v_4$ active and the other with $v_4$ inactive.
It does so by unioning $\{v_4\}$ and $\{\overline{v_4}\}$ with $\sigma_i$ and adds both new activation patterns to \mycode{ActPatterns}. Next, \bab{} loops back to process the next activation pattern.


In the second iteration, \bab{} has two subproblems. For the subproblem with activation pattern $\{v_4\}$, \mycode{Deduce} decides feasibility but \mycode{LP} returns unsatisfiable (or unknown), 
so it selects $v_2$ to split by adding $\{v_4, v_2\}$ and $\{v_4, \overline{v_2}\}$ to \texttt{ActPatterns}. 
For the other subproblem with $\overline{v_4}$ inactive, \mycode{Deduce} determines infeasibility and thus discards this subproblem.

In the third iteration, \bab{} has two subproblems corresponding to activation patterns $\{v_4, v_2\}$ and $\{v_4, \overline{v_2}\}$. For the first subproblem, \bab{} selects $v_1$ to split by unioning $v_1$ and then $\overline{v_1}$ to the current activation pattern $v_4 \land v_2$ and adds them to \mycode{ActPatterns}. For the second subproblem, \mycode{Deduce} determines infeasibility and \bab{} discards this subproblem.

In the fourth iteration, \bab{} has two subproblems corresponding to activation patterns $\{v_4, v_2, v_1\}$ and $\{v_4, v_2, \overline{v_1}\}$, both which are then determined infeasible and discarded.

Finally, \bab{} has no more activation patterns in \texttt{ActPatterns}, stops the search, and returns \unsat{}, indicating that the property is valid.

\autoref{fig:bab-example}b illustrates the BaB search tree corresponding to the above process. Each white node represents a branching decision where \bab{} splits a neuron, while each red node represents a leaf node where \mycode{Deduce} determines infeasibility and prunes the branch. Notice that not all activation patterns are explored because some branches are pruned early due to infeasibility. In other words, while there are $2^4=16$ possible complete activation patterns for the three ReLU neurons, \bab{} only explores 6 of them in this example.
\end{example}


\begin{problem}\label{prob:bab-detailed}
Do~\autoref{ex:bab} but come up with concrete values for each step, e.g., what are the bounds computed by \mycode{Deduce}, what does \mycode{LP} return, etc to illustrate the process in more detail.
\end{problem}
...
\begin{enumerate}
  \item In \autoref{alg:bab}, what happens if \mycode{Deduce} always returns infeasible?
  \item What happens if \mycode{Deduce} always returns feasible but \mycode{LP} always returns satisfiable?
...
Even the BaB search itself is optimized to avoid exploring the entire search space. For example, if \mycode{Deduce} step determines infeasibility, a smarter \bab{} variant (e.g., the \neuralsat{} tool) can analyze the conflict and add a new clause to the set of clauses (\texttt{clauses}) to prevent the same activation pattern from being selected again. This is similar to conflict-driven clause learning (CDCL) in modern SAT solvers and is implemented in the \neuralsat{} DNN verification tool~\cite{duong2025neuralsat}. In addition, heuristics are also used to select, e.g., which neuron to split next (\mycode{Decide}). We explore these optimizations and strategies in later chapters (e.g.,~\autoref{chap:neuralsat}).
...
We reuse the example in~\autoref{chap:bab} to illustrate \proofgen{}. Recall the goal is to verify that the DNN in~\autoref{fig:proof-example}(a) has the property $(x_1, x_2) \in [-2.0, 2.0] \times [-1.0, 1,0] \Rightarrow (y_1 > y_2)$. \proofgen{} generates the proof tree in~\autoref{fig:proof-example}(b) to show unsatisfiability, i.e., the property is valid.

First, \proofgen{} initializes the activation pattern set \mycode{ActPatterns} with an empty activation pattern $\emptyset$. Then \proofgen{} enters a loop (\autoref{line:dpllstart}-\autoref{line:dpllend}) to search for a satisfying assignment or a proof of unsatisfiability. In the first iteration, \proofgen{} selects the only available activation pattern $\emptyset \in \mycode{ActPatterns}$. 
It calls~\mycode{Deduce} to check the feasibility of the problem based on the current activation pattern. \mycode{Deduce} uses abstraction to approximate that from the input constraints the output values are feasible for the given network. 
Since \mycode{Deduce} cannot decide infeasibility, \proofgen{} randomly selects a neuron to split (\mycode{Decide}). Let us assume that it chooses $v_4$ to split, which essentially means the problem is split into two independent subproblems: one with $v_4$ active and the other with $v_4$ inactive.
\proofgen{} then adds $v_4$ and $\overline{v_4}$ to \mycode{ActPatterns}.

In the second iteration, \proofgen{} has two subproblems (that can be processed in parallel). For the first subproblem with $v_4$, \mycode{Deduce} cannot decide infeasibility, so it selects $v_2$ to split. It then conjoins $v_4$ with $v_2$ and then with $\overline{v_2}$ and adds both conjuncts to \texttt{ActPatterns}. 
For the second subproblem with $\overline{v_4}$ inactive, \mycode{Deduce} determines that the problem is unsatisfiable and \proofgen{} saves the node $v_4$ to the proof tree, as node 3, to indicate one unsatisfiable pattern, i.e., whenever the network has $v_4$ being inactive, the problem is unsatisfiable.

In the third iteration, \proofgen{} has two subproblems for $v_4 \land v_2$ and $v_4 \land \overline{v_2}$. For the first subproblem, \mycode{Deduce} cannot decide infeasibility, so it selects $v_1$ to split. It then conjoins $v_1$ and then $\overline{v_1}$ to the current activation pattern and adds them to \mycode{ActPatterns}. For the second subproblem, \mycode{Deduce} determines that the problem is unsatisfiable and \proofgen{} saves the node $v_4 \land \overline{v_2}$ to the proof tree, as node 5.
...
The key idea in using neuron stability is that if we can determine that a neuron is stable,  we can assign the exact truth value for the corresponding Boolean variable instead of having to guess. This has a similar effect as \mycode{BCP}---reducing mistaken assignments by \mycode{Decide}---but it operates at the theory level instead of the propositional Boolean level.
