...
%\newcommand{\tanh}[1]{\mathrm{Tanh}\left(#1\right)}%already defined in amsmath
...
\paragraph{Neural Network (NN)}  An NN can be viewed as a graph where each node represents a function that takes inputs from other nodes and produces an output.  Collectively, this graph defines a function $f$ mapping vectors of real numbers to other vectors of real numbers: 

\begin{align}
f: \mathbb{R}^{n} \to \mathbb{R}^{m}
\end{align}
where \(n\) is the dimension of the input vector and \(m\) is the dimension of the output vector. 
...
\begin{figure}[htp]
    ...
    \begin{tikzpicture}[
      ...
      \draw[->, thick] (v) -- (y);
    \end{tikzpicture}
    \caption{A simple network with 3 nodes.}
    \label{fig:snn}
\end{figure}
...
\autoref{fig:snn} shows a simple NN with three nodes: an input node (red), a hidden node (blue), and an output node (green). The input node $x$ takes a real number $r$ as input and returns the same value---i.e., an identity function \(f_x: \mathbb{R} \to \mathbb{R}\) where \(f_x(r) = r \). 

The hidden node $v$ applies a function \(f_v\) to the output $r$ of node $x$, e.g., if  $f_v(r) = 2r + 1$ then $v$ computes $2r + 1$.

Finally, the output node may also apply a function \(f_y\) to the output $r$ of node $v$, e.g., if $f_y(r) = \max(0, r)$, then the output returns the maximum of 0 and the value computed by $v$.

Thus, this NN can be represented by the function $f: \mathbb{R} \to \mathbb{R}$:
\begin{align}
f(r) = f_y(f_v(f_x(r))) = \max(0, 2r + 1)
\end{align}

\end{example}


\paragraph{Affine Transformation and Activation Function} In~\autoref{ex:nn3}, the function $f_v$ is \emph{affine}: it multiplies inputs with some constant (2) and adds a constant (1) to the result. 

The function $f_y$ is an activation function---it turns negative values into 0 and keeps positive values unchanged. In particular, the $f_y = \max(0, r)$ is the \emph{ReLU} (Rectified Linear Unit) activation function, which is widely used in DNNs.


\paragraph{Layers} An NN is typically organized into layers, where each layer consists of multiple nodes (neurons). The first layer is the input layer, which takes the input data. The last layer is the output layer, which produces the final output of the network. Between these two layers are one or more hidden layers, which perform intermediate computations.
...
\autoref{tab:activation} summarizes the most common activation functions used in DNNs, their equations, output ranges, and key uses.\tvn{need some simple piectures to demonstrate }\hai{added}
...
The piecewise linearity makes ReLU easy to reason about, but it also introduces non-linearity that complicates verification (and makes verification $\mathsf{NP}$-complete as shown in~\autoref{sec:complexity}). We will use ReLU throughout this book as the default activation function for hidden neurons in a DNN. 
...
\subsection{Sigmoid}\label{sub:sigmoid}

Sigmoid is a smooth---a continuous and differentiable---non-linear activation function that maps any real value to the range (0,1). It is often used in the output layer of a binary classification problem.
...
Sigmoid is continuous, meaning that small changes in the input will result in small changes in the output, and differentiable, meaning that it has a well-defined derivative at every point. This makes it easy to compute gradients for training the network using backpropagation. 
...
Tanh is similar to the sigmoid function but maps any real value to the range (-1,1). It is often used in the output layer of a multi-class classification problem.
...
Softmax is a generalization of the sigmoid function that maps any real value to the range (0,1) and ensures that the sum of the output values is 1. It is often used in the output layer of a multi-class classification problem.
\begin{align}
\softmax{x}_i = \frac{e^{x_i}}{\sum_{j=1}^{n}e^{x_j}}
\end{align}

\begin{example}
For a vector $x = [2, 1, 0]$, the softmax function computes: 
\begin{align*}
...
This means that the softmax function maps the input vector $x$ to a probability distribution over the three classes, where the first class has a probability of 0.71, the second class has a probability of 0.24, and the third class has a probability of 0.05.
\end{example}



\section{Example} \label{ex:e2}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=0.8, transform shape]
        ...
        \draw[->] (x1) -- node[pos=0.2] {$1.0$} (x4);
        \draw[->] (x2) -- node[pos=0.2] {$0.5$} (x3);
        \draw[->] (x2) -- node[below left=-3pt] {$1.0$} (x4);
        \draw[->] (x3) -- node[above] {$-1.0$} (x5);
        \draw[->] (x4) -- node[below] {$1.0$} (x5);

        % Output weights under x5 and x4
...
Fig.~\ref{fig:dnn} shows a simple DNN with two inputs $x_1,x_2$, one hidden layer consisting of two (hidden) neurons $x_3,x_4$, and one output $x_5$. The weights of a neuron are shown on its incoming edges, and the bias is shown above or below each neuron. 
...
\autoref{tab:nn-types} summarizes the most popular types of neural networks and their typical applications.

\begin{table}
\caption{Popular Types of Neural Networks and Their Applications}\label{tab:nn-types}
\centering
\small
...
Convolutional NN & CNN & Image recognition, video analysis, medical imaging \\
Recurrent NN & RNN & Sequence data, time series, NLP, speech recognition \\
Residual NN & ResNet & Image recognition, video analysis, medical imaging \\
% Long Short-Term Memory & LSTM & Sequence prediction, language modeling, translation \\
% Gated Recurrent Unit & GRU & Sequence modeling, time series, language tasks \\
% Radial Basis Function Network & RBFN & Function approximation, time series, control \\
% Autoencoder & -- & Dimensionality reduction, denoising, unsupervised learning \\
% Generative Adversarial Network & GAN & Data generation, image synthesis, super-resolution \\
Transformer & -- & NLP, translation, summarization, image processing \\
Graph NN & GNN & Social networks, chemistry, recommendation systems \\
\bottomrule
\end{tabular}
...
In an FFN information flows in one direction, from the input layer to hidden layers to the output layer (and thus no cycle).  

A \emph{fully connected} FFN is an FFN where every neuron in a layer is connected to every neuron in the next layer. \autoref{fig:dnn} is a fully connected FFN with one hidden layer, and \autoref{fig:ffn} is another fully connected FFN with two hidden layers. 
...
\begin{figure}[htp]
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    \caption{A Fully Connected Feed-Forward Neural Network (FNN).}\label{fig:ffn}
\end{figure}
...
CNNs, often used in image recognition and classification, consist of neurons that have learnable weights and biases~\autoref{fig:cnn}. 
Each neuron receives several inputs, takes a weighted sum over them, passes it through an activation function, and responds with an output.
...
\begin{figure}[htp]
    ...
    \foreach \i [evaluate=\i as \lbl using int(\i)] in {1,...,4} {
        \node[circle, draw, fill=white, minimum size=0.7cm, label=center:{$x_{\lbl}$}] 
            (I\i) at (\xInput, {(4-1)*0.5*\dy - (\i-1)*\dy}) {};
    }

    % Hidden nodes
    \node[circle, draw, fill=white, minimum size=0.7cm, label=center:{$h_1$}] at (\xHidden, {1.5}) (H1) {};
    \node[above=0.1cm of H1] (B1) {$b=0.5$};
    \node[circle, draw, fill=white, minimum size=0.7cm, label=center:{$h_2$}] at (\xHidden, {0}) (H2) {};
    % \node[above=0.2cm of H2] (B2) {$b=0.5$};
    \node[circle, draw, fill=white, minimum size=0.7cm, label=center:{$h_3$}] at (\xHidden, {-1.5}) (H3) {};
    % \node[above=0.2cm of H3] (B3) {$b=0.5$};

    % Output nodes
    \node[circle, draw, fill=white, minimum size=0.7cm, label=center:{$y_1$}] at (\xOutput, {1.5}) (O1) {};
    \node[circle, draw, fill=white, minimum size=0.7cm, label=center:{$y_2$}] at (\xOutput, {0}) (O2) {};
    \node[circle, draw, fill=white, minimum size=0.7cm, label=center:{$y_3$}] at (\xOutput, {-1.5}) (O3) {};

    % Connections: Input to Hidden (with weight labels spaced to avoid overlap)
    \draw[->, thick] (I1) -- node[above=2pt, midway, xshift=-2pt, yshift=1pt] {$w_1=2$} (H1);
    \draw[->, thick] (I2) -- node[below=1pt, midway, xshift=2pt] {$w_2=-1$} (H1);
    \draw[->, thick] (I2) -- (H2);
    \draw[->, thick] (I3) -- (H2);
    \draw[->, thick] (I3) -- (H3);
    \draw[->, thick] (I4) -- (H3);
    ...
    \end{tikzpicture}
    \caption{1-dimensional convolution}
    \label{fig:cnn}
\end{figure}
...
Finally, the first output is simply $y_1 = \hat{h}_1$.


\subsection{Recurrent Neural Networks (RNNs)} 

RNNs, often used in natural language process and speech recognition, are designed to recognize patterns in sequences of data. RNNs have \emph{loops} in them, allowing information to be sent forward and backward.
...
%             % Nodes
%             \node[circle, draw=black, fill=white, thick, minimum size=0.7cm] (RI) at (0,0) {};
%             \node[circle, draw=black, fill=white, thick, minimum size=0.7cm] (RH) at (2.4,0) {};
%             \node[circle, draw=black, fill=white, thick, minimum size=0.7cm] (RO) at (4.8,0) {};
%             % Main arrows
%             \draw[->, thick] (RI) -- (RH);
%             \draw[->, thick] (RH) -- (RO);
...
%             \foreach \i in {1,...,\inpN} {
%                 \node[circle, draw=black, fill=white, minimum size=0.7cm] (I\i) at (\xInput, {(\inpN-1)*0.5*\dy - (\i-1)*\dy}) {};
%             }
%             % Hidden nodes (centered) (as many as input)
%             \foreach \i in {1,...,\hidN} {
%                 \node[circle, draw=black, fill=white, minimum size=0.7cm] (H\i) at (\xHidden, {(\hidN-1)*0.5*\dy - (\i-1)*\dy}) {};
%             }
%             % Output nodes (centered) (as many as input)
%             \foreach \i in {1,...,\outN} {
%                 \node[circle, draw=black, fill=white, minimum size=0.7cm] (O\i) at (\xOutput, {(\outN-1)*0.5*\dy - (\i-1)*\dy}) {};
%             }
%             % RNN input-to-hidden (1-to-1)
%             \foreach \i in {1,...,4} {
%                 \draw[->, thick] (I\i) -- (H\i);
%             }
%             % Hidden-to-hidden (recurrent arrows, e.g. from H1 to H2, H2 to H3, etc)
%             \foreach \i [evaluate={\j=int(\i+1);}] in {1,2,3} {
%                 \draw[->, thick] ([xshift=4pt]H\i) -- ([xshift=-4pt]H\j);
%             }
%             % Hidden-to-output (1-to-1)
%             \foreach \i in {1,...,4} {
%                 \draw[->, thick] (H\i) -- (O\i);
%             }
%             % Caption
%             \node[align=center] at (3.2,-3.2) {Unrolled RNN};
%         \end{scope}
%     \end{tikzpicture}
%     \end{center}
%     \caption{RNN cell (left) and unrolled RNN sequence structure (right).\tvn{Could you add some weights, biases, ReLU or some other activation that's often used in this network to the figure to make it a complete example? Also give an example of outputs of the neurons computed using those weights and biases. Like in~\autoref{ex:e2}}}
%     \label{fig:rnn}
% \end{figure}
...
\node at (4.8,1.5) {\textbf{Output}};
% Nodes
\node[circle, draw=black, fill=white, thick, minimum size=0.7cm, label=center:{$x_t$}] (RI) at (0,0) {};
\node[circle, draw=black, fill=white, thick, minimum size=0.7cm, label=center:{$h_t$}] (RH) at (2.4,0) {};
\node[circle, draw=black, fill=white, thick, minimum size=0.7cm, label=center:{$y_t$}] (RO) at (4.8,0) {};
% Bias above nodes
\node[above=0.16cm of RH] {$0.5$};
\node[above=0.16cm of RO] {$-1$};
...
% Input nodes (left column)
\foreach \i in {1,...,\inpN} {
    \node[circle, draw=black, fill=white, minimum size=0.7cm, label=center:{$x_{\i}$}] (I\i) at (\xInput, {(\inpN-1)*0.5*\dy - (\i-1)*\dy}) {};
}
% Hidden nodes (centered) (as many as input)
\foreach \i in {1,...,\hidN} {
    \node[circle, draw=black, fill=white, minimum size=0.7cm, label=center:{$h_{\i}$}] (H\i) at (\xHidden, {(\hidN-1)*0.5*\dy - (\i-1)*\dy}) {};
    \node[above right=0 of H\i] {$0.5$}; % Bias above each hidden
}
% Output nodes (centered) (as many as input)
\foreach \i in {1,...,\outN} {
    \node[circle, draw=black, fill=white, minimum size=0.7cm, label=center:{$y_{\i}$}] (O\i) at (\xOutput, {(\outN-1)*0.5*\dy - (\i-1)*\dy}) {};
    \node[above=0.16cm of O\i] {$-1$}; % Bias above each output
}
...
Assume the input sequence is $\mathbf{x} = [x_1, x_2, x_3, x_4]$, and the initial hidden state is $h_0=0$. 
For the first time step, the hidden state is computed as $h_1 = ReLU(2x_1 - h_0 + 0.5) = ReLU(2x_1 + 0.5)$, and the output is $y_1 = h_1 - 1$. 
For the second time step, the hidden state is $h_2 = ReLU(2x_2 - h_1 + 0.5)$, and the output is $y_2 = h_2 - 1$.
% For the third time step, the hidden state is $h_3 = ReLU(2x_3 - h_2 + 0.5)$, and the output is $y_3 = h_3 - 1$.
% For the fourth time step, the hidden state is $h_4 = ReLU(2x_4 - h_3 + 0.5)$, and the output is $y_4 = h_4 - 1$.
...
%             \foreach \n/\x in {1/0, 2/2.5, 3/5, 4/7.5, 5/10} {
%                 \node[circle, draw=black, thick, minimum size=0.7cm, fill=white] (H\n) at (\x,0) {};
%             }
%             % Layer labels
...
%                 \draw[->, thick] (H\n) -- (H\next);
%             }
%             % Residual connections: skip one layer
%             \draw[->, thick] (H1) to[out=60,in=120] (H3);
%             \draw[->, thick] (H3) to[out=60,in=120] (H5);
%         \end{tikzpicture}
%     \end{center}
%     \caption{Residual Network (ResNet) block: each hidden layer receives input both from the previous layer (main path) and a skip connection from previous layers.\tvn{Could you add some weights, biases, ReLU  or some other activation that's often used in this network to the figure to make it a complete example? Also give an example of outputs of the neurons computed using those weights and biases. Like in~\autoref{ex:e2}}}
%     \label{fig:resnet}
% \end{figure}
...
\begin{figure}[htp]
    ...
    \node[circle, draw=black, thick, minimum size=0.7cm, fill=white, label=center:{$x$}] (H1) at (0,0) {};
    \node[circle, draw=black, thick, minimum size=0.7cm, fill=white, label=center:{$h_1$}] (H2) at (2.5,0) {};
    \node[circle, draw=black, thick, minimum size=0.7cm, fill=white, label=center:{$h_2$}] (H3) at (5,0) {};
    \node[circle, draw=black, thick, minimum size=0.7cm, fill=white, label=center:{$h_3$}] (H4) at (7.5,0) {};
    \node[circle, draw=black, thick, minimum size=0.7cm, fill=white, label=center:{$y$}] (H5) at (10,0) {};
    ...
    \end{tikzpicture}
    \caption{Residual Network (ResNet) block with weights on all connections and biases above nodes. Each node is labeled and typical ReLU is applied after each hidden sum.}
    \label{fig:resnet}
\end{figure}
...
\begin{aligned}
    h_1 &= \mathrm{ReLU}(x + 1) \\
    h_2 &= \mathrm{ReLU}(2h_1 + x - 1) \\
    h_3 &= \mathrm{ReLU}(-h_2 + 0.5) \\
    y   &= 0.5h_3 + h_2 + 2
\end{aligned}
\]
...
\subsection{Other properties here} %, e.g., fariness, consistency, monotonicity
...
\tvn{Need the VNN-LIB code here.}\hd{Working on it}
\end{example}
...
As shown in~\autoref{eq:prob} the NNV problem can be represented as a satisfiability problem. Specifically, we can encode the network $N$ as a logical formula $\alpha$ and use an SMT solver to check that it satisfies the property $\phi$ of interest.
...
Modern DNN verifiers (\crown{}, \neuralsat{}, etc) will first run a fast adversarial attack to check for obvious counterexamples. If one is found, the property is \sat (property violated). If no counterexample is found, the verifier will then proceed to the more expensive search-based verification algorithm (e.g., \bab{} as shown in~\autoref{chap:search-algorithms}).

\section{Random Search Attack}
Random search is a simple baseline adversarial attack.  It randomly samples points in the input domain (or within an allowed perturbation region). Each sample is checked against the property.  If any sample violates the property, a counterexample is found.
