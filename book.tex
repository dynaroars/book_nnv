...
\begin{figure}[htp]
    ...
    \foreach \i [evaluate=\i as \lbl using int(\i)] in {1,...,4} {
        \node[circle, draw, fill=white, minimum size=0.7cm, label=left:{$x_{\lbl}$}] 
            (I\i) at (\xInput, {(4-1)*0.5*\dy - (\i-1)*\dy}) {};
    }

    % Hidden nodes
    \node[circle, draw, fill=white, minimum size=0.7cm] at (\xHidden, {1.5}) (H1) {$h_1$};
    \node[above=0.1cm of H1] (B1) {$b=0.5$};
    \node[circle, draw, fill=white, minimum size=0.7cm] at (\xHidden, {0}) (H2) {$h_2$};
    % \node[above=0.2cm of H2] (B2) {$b=0.5$};
    \node[circle, draw, fill=white, minimum size=0.7cm] at (\xHidden, {-1.5}) (H3) {$h_3$};
    % \node[above=0.2cm of H3] (B3) {$b=0.5$};

    % Output nodes
    \node[circle, draw, fill=white, minimum size=0.7cm] at (\xOutput, {1.5}) (O1) {$y_1$};
    \node[circle, draw, fill=white, minimum size=0.7cm] at (\xOutput, {0}) (O2) {$y_2$};
    \node[circle, draw, fill=white, minimum size=0.7cm] at (\xOutput, {-1.5}) (O3) {$y_3$};
