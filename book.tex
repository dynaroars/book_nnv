...
\textbf{Consistency} ensures that a NN behaves consistently when given semantically equivalent or related inputs.

\begin{example}[Logical Consistency in LLMs]
    A consistency property in an LLM requires that when given a set of related queries about the same factual information, the LLM would provide logically coherent answers. Formally, consider queries $q_1$, $q_2$, and $q_3$ about a person's age:
\[
q_1: \texttt{``How old is person X?''}
\]
\[
q_2: \texttt{``What year was person X born if they are currently Y years old?''}
\]
\[
q_3: \texttt{``Will person X be Z years old in 2025?''}
\]
The consistency property requires that if the LLM outputs age $Y$ for $q_1$, then the birth year in $q_2$ should be $\texttt{current\_year} - Y$, and the answer to $q_3$ should be logically consistent with the stated age. This prevents scenarios where an LLM claims someone is 30 years old but was born in 1985 when the current year is 2024.
\end{example}
