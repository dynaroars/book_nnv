...
\begin{itemize}
    ...
    In~\autoref{fig:relu-all-abstractions}(a), the \colorbox{yellowfill}{yellow rectangle} represents the interval $[0, u_x]$.  As can be seen, this interval region is too large of an over-approximation (too coarse or loose), as it includes many points that are not achievable by ReLU, e.g., the point $(0.5, 0.0)$ is not on the red line.
...
We now define transformer functions for the interval domains for the affine function and ReLU activation.

\paragraph{Affine Function} For the affine function \(f\) in~\autoref{sec:affine}
\[f(v_1, v_2, ...,v_n) = \sum_{i}^{n}w_iv_i\]
where $n$ is number of output nodes from the previous layer. We define the transformer \(f^a\) for interval as:

\[
f^{a}([l_1, u_1],...,[l_n, u_n]) = [~\sum_{i}^{n}\min(w_iu_i, w_il_i), \sum_{i}^{n}\max(w_iu_i, w_il_i)],
\]

where \(l \in \mathbb{R}^n\) and \(u \in \mathbb{R}^n\) represent the lower \([l_1, l_2, ... l_n]\) and upper bounds \([u_1, u_2, ..., u_n]\) of the nodes of the previous layer respectively.

\begin{example}
\tvn{Hai, provide some concrete example with numbers ,e.g.,  $f^a([1, 2], [-1, 3])$ but apply it to our example DNN with weights}
\end{example}
