...
Every time verifier makes a split, it will create more constraints for each branch, e.g., $\hat{x}_i \le 0$ or $\hat{x}_i > 0$.
...
While the min/max formulation in~\autoref{sec:interval-abstraction} is intuitive, it can be efficiently implemented using matrix operations. 
