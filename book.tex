...
\]
First, reason informally. Then confirm with an SMT solver such as Z3.
\end{exercise}


\subsection{Z3 Demonstration: Logical Formulas}

We now show how to use the Python API of Z3 to check satisfiability.

\paragraph{Example 1: Simple Logic.}
\begin{verbatim}
from z3 import *
...
- The solver prints \texttt{sat}.  
- A possible model: $x = \texttt{True}, y = \texttt{True}$.

\paragraph{Exercise 11 (Homework).}  
Modify the code to check $(x \land \lnot x)$.  
Confirm that the result is \texttt{unsat}.

---

\subsection{Z3 with Linear Arithmetic}

\paragraph{Example 2: Linear Constraints.}
\begin{verbatim}
from z3 import *
...
\paragraph{Example 3: Feasible Region.}
\begin{verbatim}
s = Solver()
s.add(x + y <= 4, x >= 0, y >= 0)
print(s.check())
...
\end{verbatim}
- Output: \texttt{sat}.
- Model example: $x=0, y=0$ (Z3 returns some feasible point).

\paragraph{Exercise 12 (In-class).}  
...
\subsection{Encoding Logical Implications in Z3}

\paragraph{Example 4.}
“If $x > 3$ then $y > 2$” can be encoded as:
\begin{verbatim}
x, y = Ints('x y')
s = Solver()
...
print(s.check())
\end{verbatim}
This formula is satisfiable: for instance, $x=0, y=0$ satisfies it.

\paragraph{Exercise 13 (Homework).}  
Formulate “If $x>3$ then $y>2$” AND “$x \leq 1$” in Z3.  
Is the conjunction satisfiable? What model does Z3 return?

---

\subsection{Checking DNN-style Properties with Z3}

\paragraph{Example 5.}  
Consider a one-neuron ReLU: $y = \max(0, x-1)$.  
We want to check if the property holds: “For all $x \leq 0$, we have $y=0$.”

Encode negation: find an $x \leq 0$ with $y \neq 0$.
...
- Z3 returns \texttt{unsat}, so the property holds.

\paragraph{Exercise 14 (Homework).}  
Modify the above to check property:  
“For all $x \geq 2$, we have $y \geq 1$.”  
What does Z3 return?
