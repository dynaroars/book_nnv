...
Because our abstract domains are geometric shapes (e.g., interval, zonotope, and polytopes), it is useful to understand how these shapes can be represented. There are two common ways to describe a geometric shape:
...
\begin{example}
\autoref{fig:zonotope-correlated-generators} shows how zonotopes can capture correlations between variables. The parallelogram zonotope on the right captures   points with correlated \(x_1\) and \(x_2\) values. For example, increasing \(x_1\) by moving along \(g_1\) also increases \(x_2\) due to the non-zero second component of \(g_1\). 
In contrast, the rectangle zonotope on the left, which represents an interval abstraction, has independent generators, i.e., changing \(x_1\) does not affect \(x_2\), and therefore cannot capture such correlations.
\end{example}
