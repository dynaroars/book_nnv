...
For example, an informal specification would say

\emph{``Small changes to the input image shouldn't cause the neural network to change its classification label''. }

\noindent In contrast a formal specification of robustness would instead say:

\emph{``For an input image $x$, for all images $x'$ such that the distance between $x$ and $x'$ is at most $\epsilon$, the network's output label for $x'$ must be the same as for $x$''}
