...
To compute the abstraction of some function $f$ (e.g., activation or affine function), we need to define a \emph{transformer function} $f^a$ that takes some inputs (e.g., lower and upper bounds of the input neurons) and returns the lower and upper bounds of the output neuron after applying the function $f$. Formally, for a function $f$ that takes $n$ inputs, the transformer function $f^a$ can be defined as:
