...
In geometric terms, as shown in \autoref{fig:zonotope-vs-box}, while intervals form axis-aligned boxes, zonotopes form \emph{tilted boxes} (parallelograms or higher-dimensional analogues), preserving the direction of dependency between variables.

Thus, zonotopes extend intervals by introducing \emph{generators}, which are vectors that define directions of variability and allow the zonotope to tilt and stretch in directions that capture correlations between variables. This allows zonotopes to capture correlations between variables, leading to more precise approximations compared to intervals.
...
\paragraph{Definition}
A \textbf{zonotope} \(\mathcal{Z}\) in \(\mathbb{R}^n\) is defined as:
\[
