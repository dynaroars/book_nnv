...
As mentioned in~\autoref{sec:relu-encoding} the computation of upper and lower bounds of the neuron values is crucial for DNN verification. These bounds help determine neuron stability and therefore can significantly reduce the search space. Modern DNN verifiers explore different \emph{abstraction} techniques to compute these bounds more precisely while balancing computational efficiency.
...
We use ReLU as a running example to illustrate different abstractions.
Our goal is to compute the bounds of a post-ReLU neuron $\hat{z}$ given the bounds of its pre-ReLU value $z$ (as shown in~\autoref{sec:pre-relu-bounds}). For example, for a ReLU neuron $y = \max(0, x)$, we want to compute the bounds $[l_y(x), u_y(x)]$ of $y$ given the bounds $[l_x, u_x]$ on $x$.
...
Because our abstract domains are geometric shapes in high-dimensional spaces (i.e., multiple variables), it is useful to understand how these shapes can be represented. There are two common ways to describe a geometric shape (like rectangle, parallelograms, or higher-dimensional polyhedra):
...
\begin{example}
    ...
    x \in [-2, 3].
    \]
    We can express this interval using one generator as shown in~\autoref{fig:interval-zonotope}. 
...
A zonotope is a geometric object that generalizes the idea of an interval, which is a zonotope in 1D (\autoref{fig:interval-zonotope}), to multiple dimensions.
...
\begin{example}[2D Zonotope]
  A parallelogram is zonotope in 2D as shown in~\autoref{fig:zonotope-two-views}. It can be defined by its center point and two generator vectors that define its sides. For instance, if the center is at \(c = (1, 1)\) and the generators are \(g_1 = (2, 0)\) and \(g_2 = (0, 1.5)\), then the zonotope represents all points that can be reached by moving along these generators within the range \([-1, 1]\):
  \[
