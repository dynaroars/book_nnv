...
Note that the global robustness can clearly be trivially satisfied by labeling all points as the same class.
}
...
The interval for one variable \(x\) is defined as:
\[
x \in [l, u] = \{x \in \mathbb{R} \mid l \le x \le u\}
\]


For $n$ variables, the interval becomes a box (like a rectangle in 2D, a cuboid in 3D, or a hyperrectangle in $n$D):
\[
[x_1, x_2, \ldots, x_n] \in [l_1, u_1] \times [l_2, u_2] \times \cdots \times [l_n, u_n]
\]
...
The zonotope abstraction is more precise than interval. Unlike interval abstraction, which considers each variable independently, zonotopes capture correlations between variables (e.g., neuron activations), thereby reducing over-approximation errors. 

A zonotope represents a set as the Minkowski sum of line segments, allowing it to model linear dependencies between variables while maintaining computational efficiency.
...
Note that an interval is a special case of a zonotope where all generator vectors are axis-aligned and have the same length, i.e., \(g_i = [\pm \frac{u_i - l_i}{2}, 0, \ldots, 0]\).
