...
The \neuralsat{} approach follows the DPLL(T) framework (\autoref{sec:sat}) and is therefore an SMT solver with respect to a theory, in this case, the theory of DNNs.

Fig.~\ref{fig:overview} gives an overview of , and consists of standard DPLL components (light shades) and the theory solver (dark shade).
...
This representation allows us to  use standard DPLL components to search for truth values satisfying these clauses and a DNN-specific theory solver to check the feasibility of truth assignments---the \emph{activation pattern} (\autoref{sec:activation-pattern})---with respect to the constraints encoding the DNN and the property of interest.
...
\paragraph{DPLL(T) Iterations} \neuralsat{} searches for a satisfying \emph{activation pattern}---truth assignment for the Boolean variables to satisfy the clauses and the constraints they represent with respect to the formula in Eq.~\ref{eq:negprop}.
For this example, \neuralsat{} uses four iterations, summarized in~\autoref{tab:valid}, to determine that no such assignment exists and the problem is thus \texttt{unsat}.
...
\subsection{\tool{} vs. BnB}\label{sec:restart-tree}
...
\begin{figure}[t]
    ...
    \centering
        ...
        \tvn{Hai, instead of other verifiers can we say this search tree is for a native/general BnB approach?}
