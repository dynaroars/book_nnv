...
\begin{example}[Global Consistency: Celsius and Fahrenheit]
...
We require consistency: If Q1 outputs $y$, then Q2 must output $1.8y+32$.
...
\begin{example}[Monotonicity in Multiple Inputs: Flight Risk]
...
\begin{example}[Temporal Safety: Medical Monitoring]
...
A NN predicts admission probability to a university. Inputs: GPA $p$ and test score $s$.  
Monotonicity property: higher GPA with same score should not decrease admission probability.  
...
\begin{example}[Robustness in Speech Recognition]
...
\begin{example}[Safety in Medical Diagnosis]
...
\begin{example}[Consistency in Translation]
...
\begin{example}[Monotonicity in Fairness]
...
\begin{example}[Counterexample: Safety in Vehicles]
A NN outputs car acceleration $a$. Property: if distance to obstacle $d<10$, then $a\leq 0$.  

Observed: $d=5$ but $a=+2$.  
