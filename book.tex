...
CNNs, often used in image recognition and classification, consist of neurons that have learnable weights and biases. Each neuron receives several inputs, takes a weighted sum over them, passes it through an activation function, and responds with an output.
...
\begin{figure}[htp]
    \begin{center}
        \begin{tikzpicture}[scale=0.8, transform shape]
            ...
            % Layer labels (smaller, not overlapped)
            \node at (\xInput,{\inpN/2*\dy+0.7}) {\textbf{Input Layer}};
            \node at (\xHidden,{\inpN/2*\dy+0.7}) {\textbf{Hidden Layer}};
            \node at (\xOutput,{\inpN/2*\dy+0.7}) {\textbf{Output Layer}};
