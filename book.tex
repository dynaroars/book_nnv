...
\tvn{Hai, include some references or results, e.g., from Nguyen's, showing that SMT solvers are not scalable for DNN verification.}\hai{where can I find the results?}
...
\paragraph{ReLU Encoding} We define 
\begin{itemize}
    ...
    \item $l, u$: lower and upper bounds on $z$ over the input region
\end{itemize}
...
\begin{align*}
    ...
    &a \in \{0,1\}
\end{align*}
where $l \leq z \leq u$. Note that the constraints enforce $\hat{z} = z$ when $a = 1$ (active, $z \geq 0$) and $\hat{z} = 0$ when $a = 0$ (inactive, $z < 0$). This encoding allows the MILP solver to reason precisely about the nonlinearity using only linear constraints and binary (integer) variables.


\paragraph{General Encoding} The general MILP encoding of a DNN is then:
...
\item Step 2: Compute Pre-Activation Bounds. We compute the possible range for each pre-activation:
\begin{align*}
    z_3 &= -0.5x_1 + 0.5x_2 + 1.0 \\
    z_3^{\min} &= -0.5 \cdot 1 + 0.5 \cdot (-2) + 1 = -0.5 - 1 + 1 = -0.5 \\
    z_3^{\max} &= -0.5 \cdot (-1) + 0.5 \cdot 2 + 1 = 0.5 + 1 + 1 = 2.5 \\
    z_4 &= 0.5x_1 - 0.5x_2 - 1.0 \\
    z_4^{\min} &= 0.5 \cdot (-1) - 0.5 \cdot 2 - 1 = -0.5 - 1 - 1 = -2.5 \\
    z_4^{\max} &= 0.5 \cdot 1 - 0.5 \cdot (-2) - 1 = 0.5 + 1 - 1 = 0.5
\end{align*}
Thus, $z_3 \in [-0.5, 2.5]$ and $z_4 \in [-2.5, 0.5]$.

\item Step 3: MILP Constraints for ReLU. 
For each hidden neuron, introduce binary indicator variables $a_3, a_4 \in \{0,1\}$ and encode ReLU using the standard big-M constraints:
\begin{align*}
    &\hat{z}_3 \geq z_3 \\
    &\hat{z}_3 \geq 0 \\
    &\hat{z}_3 \leq a_3 \cdot 2.5 \\
    &\hat{z}_3 \leq z_3 + 0.5 (1 - a_3) \\
    &a_3 \in \{0,1\}
\end{align*}
\begin{align*}
    &\hat{z}_4 \geq z_4 \\
    &\hat{z}_4 \geq 0 \\
    &\hat{z}_4 \leq a_4 \cdot 0.5 \\
    &\hat{z}_4 \leq z_4 + 2.5 (1 - a_4) \\
    &a_4 \in \{0,1\}
\end{align*}

\item \textbf{Output layer constraints:}
\begin{align*}
x_5 &= -\hat{z}_3 + \hat{z}_4 - 1.0
\end{align*}

\item \textbf{Negated Output Property:}
\begin{align*}
x_5 &\le 0
\end{align*}
