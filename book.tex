...
\]
where \(g_j\) are generator, the abstract transformer \(f^a\) for the affine function is:
\[
...
\paragraph{Step 2: Apply the affine transformation.} Using the affine function \(f(x_1, x_2) = -0.5\,x_1 + 0.5\,x_2 + 1.0\),  
...
Modern DNN verifiers typically adopt the Branch-and-Bound (BaB) approach to explore the space of possible neuron activation patterns (\autoref{sec:activation-patterns}). The BaB algorithm systematically splits (\emph{branch}) the verification problem into smaller subproblems by deciding the activation status (active/inactive) of neurons, and uses abstraction (bound) techniques to compute upper and lower bounds on the output values for these subproblems. If a subproblem's bounds, computed using abstraction (\autoref{chap:abstractions}), indicate that it cannot contain a counterexample, it is pruned from the search space. This process continues until either a counterexample is found or all subproblems are exhausted, proving the property holds.
