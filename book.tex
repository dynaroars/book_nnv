...
Finally, we need to check that the generated proof is correct and indeed proves that the original DNN verification problem is indeed \unsat{}. The checker must be efficient to handle large proofs and trusted of its results (if it verifies the proof, then the original NNV problem is proved).
...
\end{gather*}
  ...
  Assume we want to check that the DNN has the property
...
As shown in~\autoref{sec:negation-trick}, we turn this into a satisfiability problem by negating the property:
...
\begin{gather}
  ...
  a_3 = 0.5x_3'' -0.2 x_4'' - x_5\label{eq:reluplex2c}
\end{gather}

In simplex terminology, a \emph{basic} variable is a variable that is used to represent the relationship between other variables in the constraints. For example, $a_1$ represents the difference between $x_1$, $x_2$, and $x_3'$. Basic variables are used to maintain the relationships between the variables in the constraints and are updated during the search process.

In contrast, a \emph{non-basic} variable is a variable that is not used to represent the relationship between other variables in the constraints. For example, $x_1$ and $x_2$ are non-basic because they are not used to represent the relationship between other variables in the constraints. Non-basic variables are typically the input variables of the DNN.
...
Observe that the lower and upper bounds of the inputs $x_1, x_2$ and output $x_5$ are specified in the property in ~\autoref{eq:negprop1}, the lower bounds of $n''$ representing ReLUs are 0s, and the other hidden variables are unbounded.

\begin{table}[h]
    \centering
\begin{tabular}{ccccccccccc}
     & $x_1$ & $x_2$ & $x_3'$ & $x_3''$ & $x_4'$ & $x_4''$ & $x_5$ &$a_1$&$a_2$&$a_3$ \\
    \midrule
...
\autoref{tab:reluplex1} shows that $x_2$ is out-of-bounds because $0 \notin [-2,-1]$. To fix $x_2$, which is non-basic, Reluplex simply updates it to a valid value, e.g., $x_2 \texttt{+=} -1.0 = -1.0$, which means adding -1.0 to the current value of $x_2$ and thus having $x_2= -1.0$.
Now, because $a_1, a_2$ depend on $x_2$ as shown in~\autoref{eq:reluplex2a} and~\autoref{eq:reluplex2b}, respectively, this update to $x_2$ also changes $a_1,a_2$:
    \begin{align*}
        a_1 ~\texttt{+=}~ 1.0 = 1.0   &~\text{because}~ a_1=x_1 - x_2 - x_3', \\
        a_2 ~\texttt{+=}~ -1.0 = -1.0  & ~\text{because}~ a_2=x_1 + x_2 - x_4'.\\
    \end{align*}



\autoref{tab:reluplex2} shows the new configuration, which now has $a_1, a_2$ violate their bounds and need to be fixed. Assume Reluplex picks $a_1$. To fix $a_1$, which is a basic variable, Reluplex pivots (swaps) it with one of the variables it depends on as shown in the constraint $ a_1 = x_1 - x_2 - x_3'$ in~\autoref{eq:reluplex2a}. Assume Reluplex pivots $a_1$ with $x_3'$, we get
...
~\autoref{tab:reluplex3} shows the new configuration. ~\autoref{tab:reluplex3} also shows the basic variable $a_2$ is out-of-bound.  To fix it, Reluplex pivots $a_2$ with a variable it depends on as shown in the constraint $a_2 = x_1 + x_2 - x_4'$ in Eq~\ref{eq:reluplex2b}.  Assume Reluplex pivots $a_2$ with $x_4'$, we get
...
At this point, we no longer have out-of-bound variables, but have inconsistent values for the pair of RELU variables $x_3', x_3''$. This is because $x_3'' = \max(x_3',0)$ but  we have $x_3'=1$ thus $\max(1,0)=1$, which is not $x_3''=0$.  Thus, Reluplex needs to fix either $x_3''$ or $x_3'$.

Assume Reluplex picks $x_3''$. Because $x_3''$ is non-basic, we simply update it, i.e.,  $x_3'' = \text{+} 1 = 1$.  As $a_3$ depends on $x_4''$, i.e., $ a_3 = 0.5x_3'' -0.2 x_4'' - x_5$, Reluplex also makes the change $a_3+= 0.5\times 1.0=0.5$.
...
In the new configuration, $a_3$ is out-of-bound. To fix this basic variable, we pivot $a_3$ with one of the variables $x_3'', x_4'', x_5$ because of the constraint $a_3 = 0.5x_3'' -0.2 x_4'' - x_5$ in ~\autoref{eq:reluplex2c}. Assume we pivot $a_3$ with $x_5$, we get
\begin{equation}\label{eq:reluplex7}
  x_5 = 0.5x_3'' -0.2 x_4'' - a_3
\end{equation}
...
Thus, in this example, we conclude that property in~\autoref{eq:prop1} is \emph{not valid} for the DNN in~\autoref{fig:reluplex} because for the inputs $x_1=0,v_{-12}=-1$, the DNN gives the output $x_5=0.5$, which violates the property in~\autoref{eq:prop1}.
\end{example}

\begin{problem}\label{problem:reluplex-hand}
    Reread the example in~\autoref{ex:reluplex} carefully. Then: 
    \begin{itemize}
    \item Rewrite the steps in~\autoref{ex:reluplex} on paper, showing all the calculations for each step. The goal of handwriting the steps is to help you understand how \reluplex{} works.  Take picture and submit as attachment.
    \item Provide feedback on this example.  Is it clear?  Are there any steps or terminologies that are confusing?  How can we improve the explanation?  For this part you write directly on a text file and submit.
    \end{itemize}
\end{problem}
...
\begin{itemize}
  ...
  \item Week 9 (Oct 22):
    ...
    \begin{itemize}        
        ...
        \item Quiz: TBD
    \end{itemize}
    \item Finally Project---Programming Assignment 4 (PA4))
    \begin{itemize}
...
\section{PGD Quiz Problems with Solutions}

\subsection*{Easy Problems}

\begin{problem}[PGD Step and Gradient Direction]\label{prob:pgd-easy1}
Consider 
\[
y = 3x_1 - 2x_2 + 1, \quad -1 \le x_1 \le 1, \ -1 \le x_2 \le 1.
\]
\begin{enumerate}
    \item Compute the gradient $\nabla_x y$.
    \item If you want to \emph{minimize} $y$, in which direction should you move: $+\nabla_x y$ or $-\nabla_x y$?
    \item Starting from $(x_1^{(0)}, x_2^{(0)}) = (0, 0)$ and $\eta = 0.5$, perform one PGD update (including clipping if needed).
\end{enumerate}

\begin{solution}
We have
\[
\nabla_x y = \left(\frac{\partial y}{\partial x_1}, \frac{\partial y}{\partial x_2}\right) = (3, -2).
\]
To minimize $y$, we move in direction $-\nabla_x y = (-3, 2)$.  
Starting from $(0, 0)$:
\[
x_1^{(1)} = 0 - 0.5 \times 3 = -1.5 \Rightarrow \text{clip to } -1, \qquad
x_2^{(1)} = 0 - 0.5 \times (-2) = 1.
\]
Thus, $(x_1^{(1)}, x_2^{(1)}) = (-1, 1)$.  
Compute
\[
y = 3(-1) - 2(1) + 1 = -4 < 0,
\]
so PGD finds a counterexample in one step.
\end{solution}
\end{problem}

\begin{problem}[Gradient Intuition]\label{prob:pgd-easy2}
Suppose $\nabla_x y = (4, -1.5)$ at some input $(x_1, x_2)$.  
Which variable, $x_1$ or $x_2$, affects $y$ more strongly, and why?

\begin{solution}
Because $|4| > |{-1.5}|$, $x_1$ affects $y$ more strongly — small changes in $x_1$ cause larger changes in $y$ than the same changes in $x_2$.
\end{solution}
\end{problem}
...
\begin{problem}[PGD with Projection]\label{prob:pgd-med1}
Consider 
\[
y = -x_1 + 2x_2 + 0.5, \quad -1 \le x_1, x_2 \le 1.
\]
We want to find a counterexample to $y > 0$ using PGD starting from $(x_1^{(0)}, x_2^{(0)}) = (0, 0)$.
\begin{enumerate}
    \item Compute the gradient $\nabla_x y$.
    \item Use step size $\eta = 1.0$ and perform one PGD update.
    \item Apply projection (clipping) if necessary.
    \item Compute $y$ at the new point. Has a counterexample been found?
\end{enumerate}
...
y = -1 + 2(-1) + 0.5 = -2.5 < 0.
\]
Thus, a counterexample is found after one iteration.
\end{solution}
\end{problem}

\begin{problem}[Choosing Step Size]\label{prob:pgd-med2}
Let 
\[
y = 2x_1 - 3x_2 + 1, \quad -1 \le x_1, x_2 \le 1.
\]
Starting from $(x_1^{(0)}, x_2^{(0)}) = (0, 0)$:
\begin{enumerate}
    \item For $\eta = 0.25$, perform two PGD iterations and check if $y \le 0$.
    \item For $\eta = 1.0$, perform one PGD iteration.  
    Which step size finds a counterexample faster?
\end{enumerate}
...
For $\eta = 0.25$:
\[
x^{(1)} = (0, 0) - 0.25(2, -3) = (-0.5, 0.75), \quad y = 2(-0.5) - 3(0.75) + 1 = -2.25 < 0.
\]
Counterexample found after 1 step.  
For $\eta = 1.0$:
\[
x^{(1)} = (0, 0) - 1.0(2, -3) = (-2, 3) \Rightarrow \text{clip to } (-1, 1),
\]
so $(x_1, x_2) = (-1, 1)$, and
\[
y = 2(-1) - 3(1) + 1 = -4 < 0.
\]
Both find a counterexample, but $\eta = 1.0$ overshoots while $\eta = 0.25$ converges smoothly.
\end{solution}
\end{problem}
...
\begin{problem}[Nonlinear Gradient Update]\label{prob:pgd-hard1}
Let 
\[
y = x_1^2 - 4x_1x_2 + 2x_2^2 + 1, \quad -2 \le x_1, x_2 \le 2.
\]
\begin{enumerate}
    \item Compute the gradient $\nabla_x y = \left( \frac{\partial y}{\partial x_1}, \frac{\partial y}{\partial x_2} \right)$.
    \item Starting from $(x_1^{(0)}, x_2^{(0)}) = (1, 1)$ and $\eta = 0.5$, perform one PGD update to minimize $y$.
    \item Clip the result if it goes out of range, and compute the new value of $y$.
    \item Discuss whether the gradient direction still aligns with the direction of decreasing $y$ after this step.
\end{enumerate}

\begin{solution}
\[
\frac{\partial y}{\partial x_1} = 2x_1 - 4x_2, \qquad
\frac{\partial y}{\partial x_2} = -4x_1 + 4x_2.
\]
At $(1,1)$:
\[
\nabla_x y = (2(1) - 4(1), -4(1) + 4(1)) = (-2, 0).
\]
Update:
\[
x^{(1)} = (1, 1) - 0.5(-2, 0) = (2, 1).
\]
Clip (still within $[-2, 2]$).  
Compute
\[
y(2, 1) = 2^2 - 4(2)(1) + 2(1)^2 + 1 = 4 - 8 + 2 + 1 = -1 < y(1, 1)=0.
\]
Thus $y$ decreases — gradient direction aligns with the desired decrease.
\end{solution}
\end{problem}

\begin{problem}[Multiple Iterations and Convergence]\label{prob:pgd-hard2}
Consider 
\[
y = x_1^2 + x_2^2 - 2x_1 - 4x_2 + 4, \quad -3 \le x_1, x_2 \le 3.
\]
\begin{enumerate}
    \item Derive $\nabla_x y$.
    \item Start from $(x_1^{(0)}, x_2^{(0)}) = (0, 0)$ and use $\eta = 0.5$.
    \item Perform 3 PGD iterations, showing all intermediate points (after clipping).
    \item Does PGD converge toward the minimum (or violation)? Explain why.
\end{enumerate}

\begin{solution}
\[
\nabla_x y = (2x_1 - 2, 2x_2 - 4).
\]
\begin{align*}
(0, 0):\ & \nabla y = (-2, -4), & x^{(1)} &= (0, 0) - 0.5(-2, -4) = (1, 2) \\
(1, 2):\ & \nabla y = (0, 0), & x^{(2)} &= (1, 2) \\
\end{align*}
The process converges after one step because $(1, 2)$ is the true minimum (solves $\nabla y = 0$).  
Thus PGD converges to the optimal violation point quickly.
\end{solution}
\end{problem}

\begin{problem}[Effect of Step Size on Overshooting]\label{prob:pgd-hard3}
Let 
\[
y = 3x_1 - 4x_2 + 2, \quad -1 \le x_1, x_2 \le 1.
\]
Starting from $(x_1^{(0)}, x_2^{(0)}) = (0, 0)$:
\begin{enumerate}
    \item Compute one PGD update with $\eta = 0.2$ and another with $\eta = 1.0$.
    \item Compare the updated points and $y$ values.
    \item Explain why a larger $\eta$ might lead to faster or less stable convergence.
\end{enumerate}
...
For $\eta = 0.2$:
\[
x^{(1)} = (0, 0) - 0.2(3, -4) = (-0.6, 0.8), \quad y = 3(-0.6) - 4(0.8) + 2 = -3.4 < 0.
\]
For $\eta = 1.0$:
\[
x^{(1)} = (0, 0) - 1.0(3, -4) = (-3, 4) \Rightarrow \text{clip to } (-1, 1),
\]
\[
y = 3(-1) - 4(1) + 2 = -5 < 0.
\]
Large $\eta$ converges faster but risks overshooting or oscillating; small $\eta$ is slower but more stable.
\end{solution}
\end{problem}
