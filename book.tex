...
It then calls \functiontextformat{Deduce} to \emph{quickly} determine if the current problem, i.e., the original satisfiability problem with  activation pattern $\sigma_i$,
...
\begin{itemize}
\item If \functiontextformat{Deduce} determines that the problem is infeasible, \bab{} discards the current processing activation pattern and loops back (to~\autoref{line:babstart}) to process the next activation pattern. In other words, it prunes the current search branch with activation pattern $\sigma_i$.
...
\item If \functiontextformat{Deduce} determines that the problem is feasible, \bab{} checks the satisfiability of the problem using an LP solver (\autoref{line:bablp})\footnote{The problem is formulated as a MILP problem as described in~\autoref{sec:milp-formulation} and checked using an LP solver. Note that we use LP and MILP interchangeably here for simplicity. In practice, most LP solvers such as Gurobi handles MILP problems as well. Just like SMT solvers handle both SAT and SMT problems.}. If the solver finds a satisfying assignment, it returns \sat{} and the counterexample represented by the satisfying assignment (\autoref{line:babsat}), indicating that the property is invalid.
Otherwise, \functiontextformat{LP} returns unsatisfiable, and \bab{} calls \functiontextformat{Decide} to select a neuron $v_i$ to split, i.e., to fix the activation status of $v_i$ as either active or inactive (\autoref{line:babdecide}).  This means the problem is split into two independent subproblems: one with $v_i$ (active) and the other with $\overline{v_i}$ (inactive). \bab{} then adds the two new activation patterns $\sigma_i \land v_i$ and $\sigma_i \land \overline{v_i}$ to \texttt{ActPatterns}. \bab{} then loops back to~\autoref{line:babstart} to process the next activation pattern.
\end{itemize}


\paragraph{\textbf{Deduce vs. LP}} Note the difference between \functiontextformat{Deduce} and \functiontextformat{LP} is that the former uses abstraction to compute over-approximated bounds to determine feasibility, while the latter uses an LP solver to check exact satisfiability. 
Abstraction is quick but may yield false positives (declaring feasible when it is not), while LP is precise but more computationally expensive.  The combination allows \bab{} to efficiently prune infeasible branches while accurately checking for counterexamples.
