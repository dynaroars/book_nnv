...
\section{Neural Network as a Function}\label{sec:nn-function}


A neural network can be viewed as a graph where each node carries out a specific operation. Collectively, this graph defines a function $f$ mapping vectors of real numbers to other vectors of real numbers: 
...
\autoref{fig:snn} shows a simple NN with 3 nodes: an input node (red), a hidden node (blue), and an output node (green). The input node takes a real number \(x\) as input and returns the same value \(x\)---i.e., an identity function \(f_x: \mathbb{R} \to \mathbb{R}\) where \(f_x(x) = x\). 

The hidden node $v$ applies some function \(f_v\) to \(x\), e.g., if  $f_v(x) = 2x + 1$ then $v$ computes $2x + 1$.

Finally, the output node may also apply another function \(f_y\) to the output of the hidden node $v$, e.g., if $f_y(x) = \max(0, x)$, then the output returns the maximum of 0 and the value computed by $v$.  

Thus, this network can be represented by the function $f: \mathbb{R} \to \mathbb{R}$:
\begin{align}
