...
\begin{enumerate}
    ...
    \item Week 2: HW: Introducing yourself on Canvas.  
    \item Week 3: 
    \item Week 3:    
    \item Week 4:
    \item Week 5:
...
\chapter{Logic and Linear Programming Foundations for DNN Verification}
...
This chapter provides the necessary background on both topics in detail, starting with propositional logic and satisfiability, 
...
\begin{itemize}
  \item A formula $\phi$ is \emph{satisfiable} if there exists some $\alpha$ with $\alpha \models \phi$.
  \item A formula $\phi$ is \emph{unsatisfiable} if no assignment satisfies it.
  \item A formula $\phi$ is \emph{valid} if all assignments satisfy it.
\end{itemize}
...
\paragraph{Exercise 2 (Homework).}  
Show that $(\phi \to \psi)$ is equivalent to $(\lnot \phi \lor \psi)$.  
(Hint: construct a truth table.)

\subsection{Conjunctive Normal Form (CNF)}

A formula is in \emph{Conjunctive Normal Form (CNF)} if it is a conjunction of disjunctions of literals,
...
CNF is important because most SAT solvers require CNF input.
Any formula can be transformed into CNF using logical equivalences.

\paragraph{Exercise 3 (In-class).}  
Transform $(x \to y) \land (y \to z)$ into CNF.

\subsection{SAT Solvers}

A SAT solver takes a CNF formula as input and determines whether it is satisfiable.
Modern solvers (e.g., MiniSAT, Glucose) use advanced algorithms such as 
\emph{DPLL (Davis–Putnam–Logemann–Loveland)} and 
\emph{CDCL (Conflict-Driven Clause Learning)}.

\paragraph{Example (SAT vs UNSAT).}  
\begin{align*}
\phi_1 &= (x \lor y) \land (\lnot x \lor z) \quad \Rightarrow \text{SAT}, \\
\phi_2 &= (x) \land (\lnot x) \quad \Rightarrow \text{UNSAT}.
\end{align*}

\paragraph{Exercise 4 (Homework).}  
Use a SAT solver (such as \texttt{pysat} in Python) to check whether
$(x \lor y) \land (\lnot x \lor y) \land (x \lor \lnot y)$ is satisfiable.  
Provide a satisfying assignment if it exists.

\subsection{Why Logic for DNN Verification?}

In DNN verification, properties such as \emph{“the network does not misclassify under small perturbations”} 
can be translated into logical formulas. 
If the formula is UNSAT, the property holds universally.
If it is SAT, the satisfying assignment corresponds to an adversarial input.

---

\section{Linear Programming (LP)}

\subsection{Linear Constraints and Feasible Regions}
...
\section{Connecting Logic and Optimization}

SAT solvers excel at discrete logic.  
LP solvers excel at continuous optimization.  
MILP solvers combine both — exactly what is needed for DNNs with piecewise-linear activations.

\paragraph{Takeaway.}
- Logic lets us model \emph{what must hold}.
- LP lets us model \emph{how quantities relate}.
- MILP encodes both — the foundation for most exact DNN verifiers today.
