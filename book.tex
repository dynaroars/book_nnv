...
Thus, the maximum value of $P$ is 945.45, achieved at the vertex $(\frac{280}{11},\frac{40}{11})$. In other words, the company should produce 24 chairs and 4 tables per week.
...
To encode binary variables in MILP, we typically use 0-1 integer variables. For example, $z \in \{0,1\}$ is a binary variable indicating whether a certain condition is met.  Moreover, we often use a ``trick'' with a large constant $M$ to encode logical implications.

\begin{example}
To encode ``if $z = True$ then $x \geq 5$'', use:
\[
...
This works because 
\begin{itemize}
 \item $z=1 \implies x \geq 5$.
 \item $z=0 \implies x \geq 5 - M$ (vacuous because $M$ is large).
\end{itemize}
...
\end{example}
\begin{exercise}
We extend the ReLU example in \autoref{ex:relu_milp} so that $x$ has the bounds $L \leq x \leq U$. Modify the MILP encoding so that $L$ and $U$ are used \emph{directly} instead of a large constant $M$.
