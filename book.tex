...
Activation functions like ReLU are non-affine and do not preserve zonotope structure. For the ReLU function \(\text{ReLU}(x) = \max(0, x)\), we must use conservative approximations. Given an input zonotope, we first compute its interval bounds \([l, u]\) (\autoref{sec:interval-affine-transformer}), then consider three cases:
