...
\subsection{Computing bounds}\label{sec:pre-relu-bounds} 

The mentioned lower and upper bounds (\autoref{sec:relu-encoding}) are critical not only for making the MILP encoding tight, but also for ensuring that the binary indicator $a$ can only take on values that are valid given the possible range of $z$. For example, if $u < 0$, then $z$ can never be non-negative, so the active phase ($a=1$) is infeasible (the constraints are not satisfiable) and should be ruled out by the MILP encodin.  Similarly, if $l \geq 0$, only the active phase is possible. Being able to eliminate infeasible cases is crucial for the efficiency of the MILP solver, as it significantly reduces the search space. 
...
\begin{enumerate}
    ...
    \item Write down the final MILP encoding of $\alpha \land \phi_{in}$.
    \item Use an MILP solver, e.g., Z3 or any other,
    to check the feasibility of the MILP encoding and report the result.
\end{enumerate}
\end{problem}
...
Suppose we have $v_1 \in [0, 1]$, $v_2 = -v_1$, and $z = v_1 + v_2$.%\tvn{Hai, do we have these kinds of correlation in DNN verification?}\hd{No, we don't.}. 
The concrete value of $z$ is always 0, but interval abstraction gives $z \in [-1, 1]$, which is a very loose over-approximation. 
