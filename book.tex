...
\autoref{tab:nn-types} summarizes the most popular types of neural networks and their typical applications. \tvn{need some simple pictures (like \autoref{fig:dnn}) to illustrate each type used in DNN verif..}\hai{added}
...
\midrule
Feedforward Neural Network & FNN/MLP & General function approximation, tabular data \\
Convolutional Neural Network & CNN & Image recognition, video analysis, medical imaging \\
Recurrent Neural Network & RNN & Sequence data, time series, NLP, speech recognition \\
Residual Neural Network & ResNet & Image recognition, video analysis, medical imaging \\
...
Graph Neural Network & GNN & Social networks, chemistry, recommendation systems \\
\bottomrule
\end{tabular}
...
A \emph{fully connected} FFN is an FFN where every neuron in a layer is connected to every neuron in the next layer. \autoref{fig:ffn} shows a simple fully connected FFN with two hidden layers, and the DNN example in~\autoref{fig:dnn} is also fully connected.
...
\begin{figure}[htp]
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    \caption{1-dimensional convolution: each hidden node receives input from a local window; each output comes from a hidden node.}
    \label{fig:conv1d}
\end{figure}
...
\begin{figure}[htp]
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    \caption{Residual Network (ResNet) block: each hidden layer receives input both from the previous layer (main path) and a skip connection from previous layers.}
    \label{fig:resnet}
\end{figure}
...
ReLU-based NNV is \NP{}-complete~\cite{katz2017reluplex,salzer2023reachability}. This means that the problem of checking whether a given ReLu-based DNN satisfies a property is computationally hard, and no polynomial-time algorithm is known to solve it in the general case.
